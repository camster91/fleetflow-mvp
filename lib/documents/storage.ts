...
export interface MalwareScanner {
  ...
  ): Promise<{ status: string; authoritativeDocumentSafety?: boolean }>
}
...
export class AdapterRegistry<T> {
  ...
  register(name: string, factory: (env: Env) => T): this {
    if (!adapterName.test(name) || this.factories.has(name)) throw new Error('Invalid or duplicate document adapter')
    this.factories.set(name, factory)
    return this
  }
  create(name: string, env: Env): T {
    const factory = this.factories.get(name)
    if (!adapterName.test(name) || !factory) throw new Error(`Document adapter '${name}' is not registered`)
    return factory(env)
  }
}

export const documentScannerRegistry = new AdapterRegistry<MalwareScanner>().register('disabled', (env) => ({
  scan: async () => {
    if (env.NODE_ENV === 'production') throw new Error('A malware scanner adapter is required in production')
    return { status: 'SKIPPED' }
  },
}))
export const documentStorageRegistry = new AdapterRegistry<DocumentStorage>().register('filesystem', (env) => {
  if (!env.DOCUMENT_STORAGE_PATH) throw new Error('DOCUMENT_STORAGE_PATH is required')
  return new PrivateDocumentStorage(env.DOCUMENT_STORAGE_PATH)
})
...
): Promise<{ status: 'CLEAN' | 'SKIPPED'; authoritativeDocumentSafety?: true }> {
  const controller = new AbortController()
  const timer = setTimeout(() => controller.abort(), Math.min(60_000, Math.max(10, options.timeoutMs)))
  try {
    const abort = new Promise<never>((_resolve, reject) =>
      controller.signal.addEventListener('abort', () => reject(new Error('Malware scan timed out')), { once: true })
    )
    const verdict = await Promise.race([scanner.scan(bytes, { signal: controller.signal }), abort])
    if (
      !verdict ||
      !['CLEAN', 'SKIPPED'].includes(verdict.status) ||
      (options.production && verdict.status !== 'CLEAN')
    )
      throw new Error('Malware scan did not return a clean verdict')
    return {
      status: verdict.status as 'CLEAN' | 'SKIPPED',
      ...(verdict.authoritativeDocumentSafety === true ? { authoritativeDocumentSafety: true as const } : {}),
    }
  } finally {
    clearTimeout(timer)
  }
...
export class PrivateDocumentStorage {
  constructor(private readonly root: string) {
    ...
    if (resolved === publicRoot || resolved.startsWith(`${publicRoot}${path.sep}`))
      throw new Error('Document storage must not be inside the public directory')
  }
    ...
    try {
      ...
      if (['EEXIST', 'EPERM'].includes((error as NodeJS.ErrnoException).code || '')) {
        try {
          ...
          /* Cleanup can be retried by operations tooling. */
        }
        return
      }
      try {
  ...
  read(storageKey: string) {
    return readFile(privateDocumentPath(this.root, storageKey))
  }
  async delete(storageKey: string) {
    try {
      await unlink(privateDocumentPath(this.root, storageKey))
    } catch (error) {
