...
const BLOCKED_KEYS = /^(?:email|phone|address|contact|contactperson|accesstoken|token|secret|password|apikey|authorization|cookie|notes?)$/i
...
export function redactSensitiveText(value: string): string {
  ...
  result = result.replace(/\b(?:api[_ -]?key|access[_ -]?token|secret|password|token)\s*[:=]\s*["']?[^\s,;"']+["']?/gi, '[REDACTED_SECRET]')
  ...
  result = result.replace(/(?:\+\d{1,3}[\s.-]*)?(?:\(\d{2,4}\)[\s.-]*|\d{3}[\s.-]+)\d{3}[\s.-]+\d{4}\b/g, '[REDACTED_PHONE]')
  return result
}
...
const SOURCE_KEYS = new Set([
  'id', 'severity', 'title', 'explanation', 'recommendedAction', 'action', 'score',
  'type', 'confidence', 'ruleVersion', 'actionUrl', 'status', 'statusValid', 'feedback',
  'feedbackValid', 'generatedAt', 'expiresAt', 'resolvedAt', 'evidence', 'evidenceValid',
  'evidenceTotal', 'evidenceTruncated', 'expired', 'effectiveStatus',
  'email', 'phone', 'address', 'contact', 'contactPerson', 'accessToken', 'token', 'secret',
  'password', 'apiKey', 'authorization', 'cookie', 'note', 'notes',
])
...
export function projectSafeFindings(input: unknown): SafeFinding[] {
  ...
  const projected = input.map((raw, index) => {
    ...
    return {
      ...
      explanation: typeof source.explanation === 'string' ? redactSensitiveText(source.explanation) : source.explanation,
      recommendedAction: typeof (source.recommendedAction ?? source.action) === 'string'
  ...
  if (!parsed.success) throw new Error(`Invalid AI findings: ${parsed.error.errors.map((issue) => issue.message).join(', ')}`)
  return parsed.data
}
