...
export const aiTelemetryEventSchema = z.object({
  ...
  errorCode: z.enum(['provider_disabled', 'provider_misconfigured', 'provider_auth', 'provider_request', 'provider_unavailable', 'provider_refusal', 'provider_incomplete', 'timeout', 'caller_aborted', 'input_invalid', 'input_too_large', 'response_too_large', 'output_malformed', 'output_invalid', 'citation_invalid', 'no_findings']).optional(),
...
}).strict().default({ enabled: false, retentionDays: 30 })

export function resolveAiRuntimeControl(settings: { enabled: boolean; killSwitch: boolean }, env: Record<string, string | undefined>) {
  if (/^(1|true|yes|on)$/i.test(env.AI_KILL_SWITCH?.trim() ?? '')) return { enabled: false as const, reason: 'global_kill_switch' as const }
...
export function providerReadiness(provider:string,modelVersion:string,env:Record<string,string|undefined>){
  ...
  if(provider!=='openai'||!env.OPENAI_API_KEY?.trim()||!env.OPENAI_MODEL?.trim()||env.OPENAI_MODEL.trim()!==modelVersion)return{ready:false as const,reason:'provider_misconfigured' as const}
