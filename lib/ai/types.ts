...
export const safeFindingSchema = z
  .object({
...
export const safeFindingsSchema = z
  .array(safeFindingSchema)
  .max(AI_LIMITS.findings)
  .superRefine((items, context) => {
    const seen = new Set<string>()
    items.forEach((item, index) => {
      if (seen.has(item.id))
        context.addIssue({ code: z.ZodIssueCode.custom, path: [index, 'id'], message: 'duplicate finding id' })
      seen.add(item.id)
    })
  })
...
export const generatedFleetSummaryContentSchema = z
  .object({
...
export const emptyFleetSummaryContentSchema = z
  .object({
...
export const fleetSummaryContentSchema = z.union([generatedFleetSummaryContentSchema, emptyFleetSummaryContentSchema])

export const fleetSummaryRequestPayloadSchema = z
  .object({
...
export const fleetSummaryJsonSchema = {
  ...
  required: ['sections', 'claims', 'actions'],
  properties: {
    sections: {
      type: 'array',
      minItems: 1,
      maxItems: AI_LIMITS.outputItems,
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['heading', 'summary', 'citationIds'],
        properties: {
          ...
          citationIds: {
            type: 'array',
            minItems: 1,
            maxItems: AI_LIMITS.citationIds,
            uniqueItems: true,
