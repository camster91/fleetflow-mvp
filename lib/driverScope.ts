...
export const DRIVER_VEHICLE_SELECT = {
  id: true,
  name: true,
  status: true,
  driver: true,
...
export const DRIVER_MAINTENANCE_SELECT = {
  id: true,
  vehicleName: true,
  vehicleId: true,
  title: true,
...
export function driverDeliveryDto(delivery: SafeRecord) {
  return {
    ...
    assignedDriverId: delivery.assignedDriverId ?? null,
    ...
    parkingInstructions: delivery.parkingInstructions ?? null,
    dropoffInstructions: delivery.dropoffInstructions ?? null,
  }
}
export function driverVehicleDto(vehicle: SafeRecord) {
  return {
    ...
    fuelLevel: vehicle.fuelLevel ?? null,
...
export function driverMaintenanceDto(task: SafeRecord) {
  ...
  return {
    id: task.id,
    vehicle: task.vehicleName ?? vehicle?.name ?? '',
