...
export function nextStagedRecord(
  existing: { payloadHash: string; reviewStatus: string; revision: number },
  nextHash: string
) {
  if (existing.payloadHash === nextHash)
    return { changed: false, revision: existing.revision, reviewStatus: existing.reviewStatus, conflictReason: null }
...
export interface StageRecordInput {
  ...
  initialConflictReason?: string | null
}
  ...
  if (!existing)
    return tx.integrationRecord.create({
      data: {
        ...
        reviewStatus: input.initialStatus || 'PENDING_REVIEW',
        conflictReason: input.initialConflictReason ?? null,
      },
    })
  const transition = nextStagedRecord(existing, input.payloadHash)
  return tx.integrationRecord.update({
    ...
    data: transition.changed
      ? {
          ...
          conflictReason: transition.conflictReason,
