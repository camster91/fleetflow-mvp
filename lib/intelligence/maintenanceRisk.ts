...
export const MAINTENANCE_RISK_RUBRIC = {
  ...
  thresholds: {
    ...
    mileageSinceService: [
      { min: 5_000, points: 6 },
      { min: 10_000, points: 12 },
    ],
    repeatedOpenCategory: [
      { min: 2, points: 8 },
      { min: 3, points: 12 },
    ],
    recordedCostIncrease: [
      { minPercent: 25, minAmount: 100, points: 10 },
      { minPercent: 75, minAmount: 100, points: 15 },
    ],
    vehicleAge: [
      { min: 8, points: 5 },
      { min: 12, points: 10 },
...
function taskLink(id: string) {
  return `/maintenance?record=${encodeURIComponent(id)}`
}
...
function pushMissing(items: string[], message: string) {
 ...
 */
export function scoreMaintenanceRisk(
  ...
  const vehicleName =
    typeof input.vehicle.name === 'string' && input.vehicle.name.trim()
      ? input.vehicle.name.trim().slice(0, 160)
      : 'Unnamed vehicle'
  ...
  if (!sourceComplete)
    pushMissing(missingData, 'Maintenance task evidence was truncated; this score may omit contributing records.')

  const validTasks = suppliedTasks.slice(0, 500)
  const open = validTasks.filter((task) => task.completed === false)
  const datedOpen = open
    .map((task) => ({ task, id: taskId(task), due: exactDate(task.dueDate) }))
    .filter((row) => row.id && row.due) as Array<{ task: TaskInput; id: string; due: Date }>
  ...
  if (overdue.length) {
    ...
    if (points)
      factors.push({
        ...
        points,
        evidence: `${overdue.length} open task${overdue.length === 1 ? '' : 's'}; oldest is ${maxDays} full day${maxDays === 1 ? '' : 's'} overdue.`,
        sourceIds: ids,
        links: ids.map(taskLink),
      })
  }
  ...
  if (serviceMileage === null || (mileage !== null && serviceMileage > mileage))
    pushMissing(missingData, 'Valid mileage at last service is unavailable.')
  ...
  if (mileage !== null && serviceMileage !== null && serviceMileage <= mileage) {
    ...
    if (points)
      factors.push({
        code: 'mileage-since-service',
        label: 'Mileage since recorded service',
        points,
        evidence: `${delta.toLocaleString('en-CA')} recorded distance units since service.`,
        sourceIds: [vehicleId],
        links: [`/vehicles/${encodeURIComponent(vehicleId)}`],
      })
  }
  ...
  const repeated = [...categories.entries()]
    .filter(([, ids]) => ids.length >= 2)
    .sort((a, b) => b[1].length - a[1].length || a[0].localeCompare(b[0]))[0]
  if (repeated) {
    const ids = repeated[1].sort()
    factors.push({
      code: 'repeated-category',
      label: 'Repeated open task category',
      points: pointsAt(ids.length, MAINTENANCE_RISK_RUBRIC.thresholds.repeatedOpenCategory),
      evidence: `${ids.length} open tasks share the “${repeated[0]}” category.`,
      sourceIds: ids,
      links: ids.map(taskLink),
    })
  }
  ...
  const completedCosts = completedRows
    .map((task) => ({
      id: taskId(task),
      date: exactDate(task.completedDate),
      cost: finiteNumber(task.actualCost, 0, MAX_COST),
      completed: task.completed,
    }))
    .filter((row) => row.id && row.date && row.cost !== null) as Array<{ id: string; date: Date; cost: number }>
  ...
  if (completedCosts.length < 4)
    pushMissing(missingData, 'At least four valid completed maintenance cost observations are required.')
  const invalidCostRows = completedRows.length - completedCosts.length
  if (invalidCostRows > 0)
    pushMissing(
      missingData,
      `${invalidCostRows} completed maintenance cost observation${invalidCostRows === 1 ? '' : 's'} have missing or invalid cost/date values.`
    )
  ...
  if (unitOk && currencyOk && completedCosts.length >= 4) {
    ...
    const priorAverage = avg(previous),
      recentAverage = avg(recent),
      increase = recentAverage - priorAverage
    const percent = priorAverage > 0 ? (increase / priorAverage) * 100 : 0
    const threshold = [...MAINTENANCE_RISK_RUBRIC.thresholds.recordedCostIncrease]
      .reverse()
      .find((item) => percent >= item.minPercent && increase >= item.minAmount)
    if (threshold) {
      const ids = recent.map((row) => row.id).sort()
      factors.push({
        code: 'recorded-cost-trend',
        label: 'Recorded maintenance cost trend',
        points: threshold.points,
        evidence: `Recent recorded average is ${Math.round(percent)}% higher (${input.currency} ${Math.round(increase)} increase) than the earlier recorded average.`,
        sourceIds: ids,
        links: ids.map(taskLink),
      })
    }
  ...
  if (year === null || !Number.isInteger(year))
    pushMissing(missingData, 'Vehicle year is unavailable or outside the supported range.')
  else {
    available++
    ...
    if (points)
      factors.push({
        ...
        label: 'Vehicle age',
        points,
        evidence: `${age} calendar years since model year ${year}.`,
        sourceIds: [vehicleId],
        links: [`/vehicles/${encodeURIComponent(vehicleId)}`],
      })
  }
  ...
  if (input.vehicle.maintenanceDue === true && !factors.some((factor) => factor.code === 'overdue-maintenance'))
    factors.push({
      code: 'open-maintenance-flag',
      label: 'Open maintenance flag',
      points: MAINTENANCE_RISK_RUBRIC.thresholds.openMaintenanceFlag,
      evidence:
        'The vehicle record has its maintenance-due flag set; no scored overdue-task factor already represents it.',
      sourceIds: [vehicleId],
      links: [`/vehicles/${encodeURIComponent(vehicleId)}`],
    })
  ...
  if (!lastService) pushMissing(missingData, 'Last service date must be an exact ISO timestamp.')
    ...
    available++
    if (now.getTime() - lastService.getTime() > 730 * 86_400_000)
      pushMissing(missingData, 'Last service record is more than two years old and may be stale.')
  }
