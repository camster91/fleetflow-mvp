...
function validEvidenceItem(item: unknown): item is FindingEvidence {
  ...
  return (
    ...
    EVIDENCE_ENTITY_TYPES.has(value.entityType) &&
...
function safeOperationalEvidence(item: FindingEvidence): boolean {
  ...
  if (DATE_FIELDS.has(item.field))
    return typeof item.value === 'string' && item.value.length <= 64 && Number.isFinite(new Date(item.value).getTime())
  if (NUMBER_FIELDS.has(item.field))
    return typeof item.value === 'number' && Number.isFinite(item.value) && item.value >= 0
  if (item.field === 'vehicleId')
    return item.value === null || (typeof item.value === 'string' && /^[\x21-\x7e]{1,128}$/.test(item.value))
  return false
}
...
export function parseStoredEvidence(value: unknown): ParsedFindingEvidence {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      ...
      if (
        keys.join(',') !== 'items,total,truncated' ||
...
export function packFindingEvidence(evidence: readonly FindingEvidence[]): string {
  ...
  for (const item of valid) {
    ...
    const encoded = JSON.stringify({
      items: candidate,
      total: valid.length,
      truncated: candidate.length < valid.length,
    })
...
export function presentStoredFinding(row: Record<string, unknown>, now = new Date()) {
  ...
  return {
    ...
    action: row.action,
    actionUrl: safeFindingActionUrl(row.actionUrl),
    status,
    statusValid,
    feedback: feedbackValid ? row.feedback : null,
    feedbackValid,
    generatedAt: row.generatedAt,
    expiresAt: row.expiresAt,
    resolvedAt: row.resolvedAt,
    evidence: safeEvidence,
    evidenceValid: parsed.valid && !redacted,
    evidenceTotal: redacted ? null : parsed.evidenceTotal,
    evidenceTruncated: parsed.evidenceTruncated || redacted,
    expired,
