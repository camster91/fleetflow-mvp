...
export const enrollmentSchema = z.object({
  ...
  if (value.status === 'ACTIVE' && !value.consent) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['consent'], message: 'Pilot activation requires recorded consent' })
  if (value.pilotStartsAt && value.pilotEndsAt && new Date(value.pilotEndsAt) <= new Date(value.pilotStartsAt)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['pilotEndsAt'], message: 'Pilot end must follow start' })
})
