...
function parseArgs(argv) {
  ...
  if (!SAFE_CONTAINER.test(values['source-container']))
    throw new Error('--source-container contains unsupported characters')
...
function validateRestoreParity(source, restored) {
  const errors = []
  if (source?.publicTableCount !== restored?.publicTableCount)
    errors.push('Restore public-table count differs from source')
  for (const table of REQUIRED_RESTORE_TABLES) {
  ...
  if (source?.migrations?.applied !== restored?.migrations?.applied)
    errors.push('Restore applied-migration count differs from source')
  if (source?.migrations?.failed !== restored?.migrations?.failed)
    errors.push('Restore failed-migration count differs from source')
  return errors
}
