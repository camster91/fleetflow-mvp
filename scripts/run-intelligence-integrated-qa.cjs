...
const { container, main, npx, parseFlags, psql, removeContainers, removeTempDir, run, startNpm, startPostgres, stopProcessTree, waitForHttp, waitPostgres } = require('./qa-harness-lib.cjs')

const PRE_RUNS_MIGRATIONS = ['20260807000000_postgresql_baseline', '20260808000000_stripe_webhook_idempotency', '20260808010000_api_key_scopes', '20260808020000_intelligence_findings']
const UPGRADE_SEED_SQL = 'INSERT INTO "User" ("id","email","updatedAt") VALUES (\'upgrade-user\',\'upgrade@fleetvera.test\',CURRENT_TIMESTAMP); INSERT INTO "Team" ("id","name","ownerId","updatedAt") VALUES (\'upgrade-team\',\'Upgrade fixture\',\'upgrade-user\',CURRENT_TIMESTAMP); INSERT INTO "IntelligenceFinding" ("id","ownerId","teamId","type","severity","confidence","score","ruleVersion","title","explanation","evidence","generatedAt") VALUES (\'upgrade-finding\',\'upgrade-user\',\'upgrade-team\',\'qa\',\'low\',1,1,\'qa\',\'Upgrade finding\',\'Preserved across migration\',\'{"items":[],"total":0,"truncated":false}\',CURRENT_TIMESTAMP);'
const RUN_TABLE_EXISTS = "EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'IntelligenceRun')"
...
main(async () => {
  ...
  try {
    ...
    for (const migration of PRE_RUNS_MIGRATIONS) {
      cpSync(path.join('prisma/migrations', migration), path.join(migrationFixture, 'migrations', migration), { recursive: true })
    }
    npx(['prisma', 'migrate', 'deploy', '--schema', path.join(migrationFixture, 'schema.prisma')], 'Pre-030000 fixture migration failed', env)
    ...
    const seed = psql(containerName, { user: 'fleetvera', database: 'fleetvera_qa', sql: UPGRADE_SEED_SQL, extraArgs: ['-v', 'ON_ERROR_STOP=1'] })
    ...
    const upgradeCheck = query('fleetvera_qa', `SELECT ${RUN_TABLE_EXISTS} AND EXISTS (SELECT 1 FROM "IntelligenceFinding" WHERE id = 'upgrade-finding')`)
    if (upgradeCheck !== 't') throw new Error('030000 upgrade did not create IntelligenceRun or preserve seeded findings')
    ...
    if (run('npx', ['playwright', 'test', 'e2e/intelligence-integrated.spec.ts', '--project=chromium', '--reporter=line'], { env }).status !== 0) {
      throw new Error('Integrated Playwright failed')
    }
    if (run('npx', ['playwright', 'test', 'e2e/intelligence-brief.spec.ts', '--project=chromium', '--reporter=line'], { env }).status !== 0) {
