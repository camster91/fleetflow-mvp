...
const toastStyles: Record<
  ToastType,
  { bg: string; border: string; icon: string; Icon: React.ComponentType<{ size?: number; className?: string }> }
> = {
  success: {
    bg: 'bg-white',
...
export const Toast: React.FC<ToastProps> = ({ toast, onDismiss, position = 'top-right' }) => {
    ...
    >
      <div className="flex gap-3 p-4">
        ...
        <div className={`shrink-0 ${styles.icon}`}>
          <Icon size={20} className={toast.type === 'loading' ? 'animate-spin' : ''} />
        </div>
    ...
    >
      {visibleToasts.map((toast, index) => (
        <div
          ...
          style={{
            transform:
              isStacked && index < visibleToasts.length - 1
                ? `scale(${1 - (visibleToasts.length - 1 - index) * 0.05})`
                : undefined,
            opacity:
              isStacked && index < visibleToasts.length - 1 ? 1 - (visibleToasts.length - 1 - index) * 0.15 : undefined,
          }}
          className="transition-all duration-300"
        >
          <Toast toast={toast} onDismiss={onDismiss} position={position} />
        </div>
      ))}
    </div>
  )
...
export const ToastProvider: React.FC<ToastProviderProps> = ({
  ...
  const success = useCallback(
    (message: string, options?: Omit<Partial<ToastData>, 'type' | 'message'>) => {
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <ToastContainer position={position} toasts={toasts} onDismiss={dismiss} maxToasts={maxToasts} />
