...
export const CardHeader = React.forwardRef<HTMLDivElement, CardHeaderProps>(
  ({ children, className = '', ...props }, ref) => (
    <div ref={ref} className={`px-6 py-4 border-b border-slate-100 dark:border-slate-800 ${className}`} {...props}>
      {children}
    </div>
  )
)
  ...
  return (
    <Card className={className}>
      <div className="flex items-start justify-between">
        ...
        {icon && <div className={`p-3 rounded-lg ${bgColor} ${iconColor || ''}`}>{icon}</div>}
      </div>
    </Card>
  )
}
