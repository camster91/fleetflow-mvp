...
export function Pagination({
  label, page, pageSize, total, onPageChange, onPageSizeChange, disabled = false, className = '',
}: PaginationProps) {
  ...
  return (
    ...
    >
      <p className="text-sm text-slate-500 dark:text-slate-400" aria-live="polite" aria-atomic="true">
        {total === 0 ? `No ${label}` : `Showing ${start.toLocaleString()}–${end.toLocaleString()} of ${total.toLocaleString()} ${label}`}
      </p>
      <div className="flex flex-wrap items-center gap-2">
        <label htmlFor={selectId} className="text-sm text-slate-600 dark:text-slate-300">Rows per page</label>
        <select
          id={selectId}
          value={pageSize}
          disabled={disabled}
        ...
        <button type="button" className={buttonClass} aria-label="First page" disabled={disabled || atStart} onClick={() => onPageChange(1)}>
          <ChevronsLeft className="h-4 w-4" aria-hidden="true" />
        </button>
        <button type="button" className={buttonClass} aria-label="Previous page" disabled={disabled || atStart} onClick={() => onPageChange(current - 1)}>
        ...
        <button type="button" className={buttonClass} aria-label="Next page" disabled={disabled || atEnd} onClick={() => onPageChange(current + 1)}>
          <ChevronRight className="h-4 w-4" aria-hidden="true" />
        </button>
        <button type="button" className={buttonClass} aria-label="Last page" disabled={disabled || atEnd} onClick={() => onPageChange(pageCount)}>
...
export interface SortOption { value: string; label: string; order?: 'asc' | 'desc' }

/** "Sort by" select bound to the paginated list sort state (`value|order`). */
export function SortSelect({
  id, options, sort, order, onChange,
}: {
  id: string;
  options: SortOption[];
  sort: string;
  ...
  const selected = options.find((option) => option.value === sort && (option.order ?? '') === order)
    ?? options.find((option) => option.value === sort)
    ?? options[0];
  const key = (option: SortOption) => `${option.value}|${option.order ?? ''}`;
  return (
    <div className="flex items-center gap-2">
      <label htmlFor={id} className="text-sm text-slate-600 whitespace-nowrap dark:text-slate-300">Sort by</label>
      <select
        id={id}
        value={key(selected)}
        onChange={(event) => {
