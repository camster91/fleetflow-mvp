...
export function ActionPreview({
  ...
  const preview = async () => {
    ...
    try {
      ...
      setMessage(
        "This suggested action could not be previewed. No records were changed.",
      );
  ...
  const confirm = async () => {
    ...
    try {
      ...
      setMessage(
        "The change was not applied. Refresh the preview and try again.",
      );
    } finally {
      setResponse(null);
      setBusy(false);
  ...
  const copyDraft = async () => {
    ...
    try {
      await navigator.clipboard.writeText(
        JSON.stringify(response.preview.after, null, 2),
      );
      setMessage("Draft copied to clipboard.");
    } catch {
      setMessage(
        "Copy was unavailable. Select the visible draft values instead.",
      );
    }
  };
  const cancel = () => {
