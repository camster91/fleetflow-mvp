...
export function FindingCard({ finding, canManage, canFeedback, busy = false, onAction, cardRef }: FindingCardProps) {
  ...
  const [deliveryStatus, setDeliveryStatus] = useState<'pending' | 'in-transit' | 'delivered' | 'cancelled'>(
    'in-transit'
  )
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p
            className={`inline-flex min-h-7 items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold ${severity === 'high' ? 'bg-red-50 text-red-700' : severity === 'medium' ? 'bg-amber-50 text-amber-800' : 'bg-slate-100 text-slate-700'}`}
          >
        ...
        <div id={disclosureId} className="mt-2 rounded-lg bg-slate-50 p-3 text-sm">
          {!finding.evidenceValid ? (
            <p role="status" className="text-amber-800">
              Supporting evidence is unavailable. Refresh the brief before acting.
            </p>
          ...
          {finding.evidenceTruncated && (
            <p role="status" className="mt-2 text-amber-800">
              Only part of the supporting evidence is shown.
            </p>
          )}
        </div>
      )}
        ...
        {canFeedback && (
          <>
            <button
              disabled={busy}
              type="button"
              aria-pressed={finding.feedback === 'HELPFUL'}
              aria-label={`Mark ${finding.title} helpful`}
              onClick={() => onAction(finding, 'HELPFUL')}
              className={`inline-flex min-h-11 items-center gap-1 rounded-lg px-3 text-sm hover:bg-slate-100 disabled:opacity-50 ${finding.feedback === 'HELPFUL' ? 'bg-emerald-50 text-emerald-900' : 'text-slate-700'}`}
            >
            ...
            <button
              disabled={busy}
              type="button"
              aria-pressed={finding.feedback === 'NOT_HELPFUL'}
              aria-label={`Mark ${finding.title} not helpful`}
              onClick={() => onAction(finding, 'NOT_HELPFUL')}
              className={`inline-flex min-h-11 items-center gap-1 rounded-lg px-3 text-sm hover:bg-slate-100 disabled:opacity-50 ${finding.feedback === 'NOT_HELPFUL' ? 'bg-amber-50 text-amber-900' : 'text-slate-700'}`}
            >
        ...
        {canManage && finding.effectiveStatus === 'OPEN' && (
          <>
            <button
              ...
              className="inline-flex min-h-11 items-center gap-1 rounded-lg px-3 text-sm text-slate-700 hover:bg-slate-100 disabled:opacity-50"
            >
      ...
      {canManage && finding.evidenceValid && (deliveryEvidence || vehicleEvidence) && (
        <section
          aria-label={`Suggested actions for ${finding.title}`}
            ...
            <div className="space-y-2">
              ...
              <ActionPreview
                ...
                action={{
                  ...
                  deliveryId: deliveryEvidence.entityId,
                  values: { status: deliveryStatus },
                  expectedUpdatedAt: deliveryEvidence.timestamp!,
                }}
              />
            </div>
          )}
              ...
              {maintenanceType.trim() && (
                <ActionPreview
                  ...
                  action={{
                    ...
                    vehicleId: vehicleEvidence.entityId,
                    values: {
                      vehicle: 'Authorized vehicle',
                      type: maintenanceType.trim(),
                      dueDate: today,
                      priority: finding.severity === 'high' ? 'high' : finding.severity === 'medium' ? 'medium' : 'low',
                    },
                    expectedVehicleUpdatedAt: vehicleEvidence.timestamp!,
                  }}
                />
              )}
            </div>
