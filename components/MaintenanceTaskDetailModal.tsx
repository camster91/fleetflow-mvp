...
export default function MaintenanceTaskDetailModal({
  isOpen,
  task,
  onClose,
  onUpdated,
  ...
  const priorityBadge = (p: string) => {
    if (p === 'high')
      return (
        <span className="px-2.5 py-1 bg-red-100 text-red-700 rounded-full text-xs font-semibold">High Priority</span>
      )
    if (p === 'medium')
      return (
    ...
    return (
      <span className="px-2.5 py-1 bg-slate-100 text-slate-600 rounded-full text-xs font-semibold">Low Priority</span>
    )
  }

  const inputCls =
    'w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-900 focus:border-transparent'
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {task.completed ? 'Completed' : isOverdue ? 'Overdue' : 'Scheduled'}
                </p>
              </div>
            </div>
          ...
          <div className="px-6 py-5 space-y-4">
                ...
                {isEditing ? (
                  ...
                  <p className="mt-1 text-slate-900 font-medium">
                    {task.costEstimate != null ? (
                      `$${Number(task.costEstimate).toFixed(2)}`
                    ) : (
            ...
            {task.completed && task.completedDate && (
              <div className="bg-emerald-50 rounded-xl p-3 flex items-center gap-2">
                ...
                <p className="text-sm text-emerald-700">
                  Completed on{' '}
                  {new Date(task.completedDate + 'T12:00:00').toLocaleDateString('en-US', {
                    month: 'long',
                    day: 'numeric',
                    year: 'numeric',
                  })}
            ...
            {shareUrl && (
              <div className="bg-blue-50 rounded-xl p-3">
                ...
                <div className="flex items-center gap-2">
                  <p className="text-xs text-blue-600 flex-1 truncate font-mono bg-white px-2 py-1.5 rounded-lg border border-blue-100">
                    {shareUrl}
                  </p>
                  <button
                    type="button"
