...
import { LayoutDashboard, Car, Package, Wrench, Users, BarChart3, Settings, Search, Menu, X, ChevronDown, ChevronRight, LogOut, HelpCircle, FileText, MoreHorizontal, BookOpen, ShoppingCart, Building, ClipboardList, Sparkles } from 'lucide-react';
...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children, title, subtitle, actions, breadcrumbs }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 dark:text-white">
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 z-50 flex h-full w-72 flex-col bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
