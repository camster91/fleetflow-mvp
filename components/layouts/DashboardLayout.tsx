      ...
      <div className="flex items-stretch" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        {tabs.map((tab) => {
          ...
          const isTabActive =
            ...
            (tab.href !== '/dashboard' && tab.href !== '#more' && currentPath.startsWith(tab.href))
...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  const doSearch = (q: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        if (r.ok) {
          setSearchResults((await r.json()) as SearchResults)
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 dark:text-white">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 z-50 flex h-full w-72 flex-col bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
              ...
              return (
                <div key={item.id}>
                  {hasChildren ? (
                        ...
                        <div className="mt-1 ml-4 pl-4 border-l border-slate-200 space-y-1">
                          {item.children.map((child) => (
                            <Link
                              key={child.id}
                              href={child.href}
                              className={`block px-3 py-2 rounded-lg text-sm transition-colors ${router.asPath === child.href ? 'bg-blue-50 text-blue-900 font-medium' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}
                            >
                              {child.label}
                            </Link>
                          ))}
                    ...
                    <Link
                      href={item.href || '#'}
                      className={`flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${active ? 'bg-blue-50 text-blue-900' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}
                    >
                      <Icon className={`h-5 w-5 ${active ? 'text-blue-600' : 'text-slate-400'}`} />
                      <span>{item.label}</span>
                    </Link>
                ...
                <div className="relative" ref={searchRef}>
                  ...
                  {searchDropOpen && searchResults && (
                    <div className="absolute top-full left-0 mt-1 w-80 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 z-50 overflow-hidden">
                            ...
                            {items.map((item) => (
                              ...
                              >
                                <span className="font-medium">
                                  {item.name || item.customer || item.vehicle || item.vehicleName || item.type}
                                </span>
                      ...
                      {(['vehicles', 'deliveries', 'clients', 'maintenance'] as const).every(
                        (c) => !searchResults[c]?.length
                      ) && (
