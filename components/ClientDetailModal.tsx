...
export default function ClientDetailModal({ isOpen, onClose, client, onClientUpdated, initialEditing = false }: ClientDetailModalProps) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const updated = await dataService.updateClient(client.id, editedClient)
      if (updated) {
          ...
          <div className="bg-gray-50 px-6 py-4 border-t">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600">
                <p>Client ID: {client.id} • Created: {client.created ? new Date(client.created).toLocaleDateString() : 'N/A'}</p>
              </div>
              <div className="flex items-center space-x-3">
                <button
                  onClick={onClose}
