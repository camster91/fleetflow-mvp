...
test('maintenance attention is explainable, keyboard usable, and fits 375px', async ({ page, baseURL }) => {
  ...
  test.skip(
    ...
    'Deterministic auth fixture is restricted to a local Fleetvera server'
  )
  ...
  await page.route('**/api/intelligence/maintenance-risk-feedback', (route) => {
    ...
    return saveAttempts === 1
      ? route.fulfill({ status: 200, contentType: 'application/json', body: 'invalid-json-simulates-lost-response' })
      : route.fulfill({ status: 200, json: { feedback: { id: 'pilot-1' }, replayed: true } })
  })
  await page.route('**/api/analytics/dashboard**', (route) =>
    route.fulfill({
      json: {
        stats: {
          fleetUtilization: { current: 50, total: 2, active: 1 },
          deliveries: { total: 0, delivered: 0, inTransit: 0, pending: 0, delayed: 0 },
          maintenance: { total: 1, overdue: 1, dueSoon: 0, completed: 0, totalCost: 0, upcoming: [] },
          clients: { total: 0 },
        },
        charts: { activityOverTime: [], maintenanceByCategory: [], vehicleUtilization: [] },
        maintenanceRisk: {
          evaluatedVehicles: 1,
          coverage: { vehiclesComplete: true, tasksComplete: true },
          items: [
            {
              vehicleId: 'v1',
              ...
              factors: [
                {
                  ...
                  points: 30,
                  evidence: 'One open task is 31 full days overdue.',
                  sourceIds: ['t1'],
  ...
  await page.getByRole('button', { name: 'Withdraw pilot feedback' }).click()
  await expect(page.getByText('Pilot feedback withdrawn.')).toBeVisible()
