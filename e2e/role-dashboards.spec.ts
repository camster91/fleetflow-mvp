...
async function mockDashboard(
  ...
  role: string,
  options: { onboardingCompleted?: boolean; unavailable?: Array<'vehicles' | 'deliveries' | 'maintenance'> } = {}
) {
  await page.addInitScript(() => localStorage.clear())
  const baseURL = test.info().project.use.baseURL as string
  const origin = new URL(baseURL)
  if (!process.env.JWT_SECRET) throw new Error('JWT_SECRET is required for role dashboard browser QA')
  const token = await new SignJWT({ sub: 'role-qa', email: 'role-qa@fleetvera.test', role })
  ...
  await page.route('**/api/auth/me', (route) =>
    route.fulfill({
      json: {
        user: { id: 'role-qa', email: 'role-qa@fleetvera.test', name: 'Role QA', role, onboardingCompleted: true },
      },
    })
  )
  const roleActions: Record<string, Array<{ label: string; href: string }>> = {
    ...
    DISPATCHER: [
      { label: 'Assign delivery', href: '/deliveries' },
      { label: 'Review active deliveries', href: '/deliveries' },
      { label: 'Check driver status', href: '/vehicles' },
    ],
    TECHNICIAN: [
      { label: 'Update work orders', href: '/maintenance' },
      { label: 'Review maintenance schedule', href: '/maintenance' },
      { label: 'Open service procedures', href: '/sop' },
    ],
    DRIVER: [
      { label: 'Open my deliveries', href: '/deliveries' },
      { label: 'View my assigned vehicle', href: '/vehicles' },
      { label: 'Open safety procedures', href: '/sop' },
    ],
    VIEWER: [],
  }
  ...
  await page.route('**/api/dashboard/context', (route) =>
    route.fulfill({
      json: {
        role,
        dashboardRole: ['OWNER', 'ADMIN', 'MANAGER'].includes(role)
          ? 'admin'
          : role === 'DISPATCHER'
            ? 'dispatcher'
            : role === 'TECHNICIAN'
        ...
        sources: {
          vehicles: unavailable.has('vehicles')
            ? { available: false, error: 'Vehicles unavailable', items: [], total: null }
            : {
                available: true,
                items: [
                  {
          ...
          deliveries: unavailable.has('deliveries')
            ? { available: false, error: 'Deliveries unavailable', items: [], total: null }
            : {
                available: true,
                items: [
                  {
          ...
          maintenance: unavailable.has('maintenance')
            ? { available: false, error: 'Maintenance unavailable', items: [], total: null }
            : {
                available: true,
                items: [
                  {
  ...
  await page.route('**/api/intelligence/brief**', (route) =>
    route.fulfill({
      json: {
        ...
        capabilities: { refresh: false, manage: false, feedback: false },
      },
    })
  )
  await page.route('**/api/intelligence/data-quality**', (route) =>
    route.fulfill({
      json: {
        issues: [],
        summary: { total: 0, bySeverity: {}, countsComplete: true },
  ...
  for (const viewport of ['desktop', '375px'] as const) {
    test(`${role} receives its bounded command centre at ${viewport}`, async ({ page }) => {
      ...
      const forbidden: Record<string, string[]> = {
        OWNER: ['Assign delivery', 'Update work orders', 'Open my deliveries'],
        DISPATCHER: ['Add vehicle', 'Manage team', 'Update work orders'],
        TECHNICIAN: ['Add vehicle', 'Manage team', 'Assign delivery'],
        DRIVER: ['Add vehicle', 'Manage team', 'Assign delivery', 'Update work orders'],
        VIEWER: ['Add vehicle', 'Manage team', 'Assign delivery', 'Update work orders', 'Open my deliveries'],
      }
...
test('denied canonical context fails closed without privileged actions', async ({ page }) => {
  await mockDashboard(page, 'OWNER')
  await page.route('**/api/dashboard/context', (route) => route.fulfill({ status: 403, json: { error: 'Forbidden' } }))
  await page.goto('/dashboard')
  await expect(page.getByText('Unable to open this workspace dashboard')).toBeVisible()
  await expect(page.getByText('Add vehicle')).toHaveCount(0)
})
...
test('admin setup checklist is functional and dismissible', async ({ page }) => {
  ...
  await page
    .getByRole('button', { name: 'Dismiss checklist' })
    .evaluate((element) => (element as HTMLButtonElement).click())
  await expect(page.getByRole('heading', { name: 'Setup Checklist' })).toHaveCount(0)
})

for (const viewport of ['desktop', '375px'] as const)
  test(`partial source truth never claims empty delivery data at ${viewport}`, async ({ page }) => {
    if (viewport === '375px') await page.setViewportSize({ width: 375, height: 812 })
