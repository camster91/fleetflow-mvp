...
test('real backend enforces signed tenant-scoped stale and replay-safe confirmation with atomic audit', async ({
  ...
  const previewResponse = await request.post(`${baseURL}/api/assistant/actions/execute`, {
    headers,
    data: {
      action: {
  ...
  expect(preview.preview.before).toEqual(expect.objectContaining({ status: 'pending', notes: 'Original' }))
  expect(preview.preview.after).toEqual(expect.objectContaining({ status: 'delivered', notes: 'Confirmed note' }))
  ...
  expect((await db.delivery.findUniqueOrThrow({ where: { id: deliveryId } })).status).toBe('delivered')
  expect(
  ...
  const stalePreviewResponse = await request.post(`${baseURL}/api/assistant/actions/execute`, {
    headers,
    data: {
      action: {
        type: 'update_delivery_status',
  ...
  const staleConfirm = await request.post(`${baseURL}/api/assistant/actions/execute`, {
    headers,
    data: { previewToken: stalePreview.token, confirm: true },
  })
  expect(staleConfirm.status()).toBe(409)
  expect((await db.delivery.findUniqueOrThrow({ where: { id: staleDeliveryId } })).status).toBe('pending')
  const outsiderJwt = await new SignJWT({
    ...
    .setExpirationTime('1h')
    .sign(new TextEncoder().encode(process.env.JWT_SECRET!))
  const crossTenant = await request.post(`${baseURL}/api/assistant/actions/execute`, {
    headers: {
      ...headers,
  ...
  const deletedSource = await request.post(`${baseURL}/api/assistant/actions/execute`, {
    headers,
    data: { previewToken: preview.token, confirm: true },
  })
