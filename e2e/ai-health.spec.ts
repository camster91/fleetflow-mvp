...
for (const viewport of ['desktop', '375px'] as const) test(`AI health controls remain accessible and bounded at ${viewport}`, async ({ page }) => {
  ...
  const token = await new SignJWT({ sub: 'qa-admin', email: 'qa@fleetvera.test', role: 'admin' }).setProtectedHeader({ alg: 'HS256' }).setIssuedAt().setExpirationTime('1h').sign(new TextEncoder().encode(process.env.JWT_SECRET))
  await page.context().addCookies([{ name: 'token', value: token, domain: origin.hostname, path: '/', httpOnly: true, sameSite: 'Lax' }])
  await page.route('**/api/auth/me', route => route.fulfill({ json: { user: { id: 'qa-admin', email: 'qa@fleetvera.test', name: 'QA Admin', role: 'admin', onboardingCompleted: true } } }))
  ...
  await page.route('**/api/admin/ai-health', route => route.fulfill({ json: { status: 'enabled', provider: 'openai', modelVersion: 'gpt-safe@v1', configVersion: 2, retentionDays: 30, requests: 3, averageLatencyMs: 200, fallbackRate: 1 / 3, inputTokens: 20, outputTokens: 10, errors: [{ code: 'timeout', count: 1 }], evaluation: { version: 'fixtures@v1', passed: true, runAt: '2026-08-08T01:00:00.000Z' } } }))
  let mutation: unknown
  await page.route('**/api/admin/ai-settings', async route => { mutation = route.request().postDataJSON(); await route.fulfill({ json: { enabled: false, killSwitch: true, retentionDays: 7, provider: 'openai', modelVersion: 'gpt-safe@v1', configVersion: 3 } }) })
