...
test('document review is usable on mobile and cancellation does not confirm', async ({ page }) => {
  ...
  const row = {
    ...
    extraction: {
      ...
      fields: {
        vendor: { value: 'Honest Garage', confidence: 0.98, citationIds: ['c1'] },
        date: { value: '2026-08-08', confidence: 0.95, citationIds: ['c2'] },
        total: { value: 120, confidence: 0.55, citationIds: ['c3'] },
      },
      services: [{ description: 'Oil change', quantity: 1, amount: 80, confidence: 0.9, citationIds: ['c1'] }],
      parts: [],
      citations: [
        { id: 'c1', page: 1, quote: 'Honest Garage' },
        { id: 'c2', page: 1, quote: '2026-08-08' },
        { id: 'c3', page: 1, quote: '120' },
      ],
      warnings: [],
    },
  }
  await page.route('**/api/auth/session', (route) =>
    route.fulfill({ json: { user: { id: 'u1', name: 'Manager', onboardingCompleted: true } }, status: 200 })
  )
  await page.route('**/api/documents/upload**', async (route) => {
    const request = route.request()
    if (request.method() === 'POST') {
      uploaded = true
      return route.fulfill({ status: 201, json: { document: row, duplicate: false } })
    }
  ...
  await page.route('**/api/documents/d1/extract', async (route) => {
    ...
    if (body.action === 'preview')
      return route.fulfill({
        json: {
          previewToken: 'signed-preview-token-that-is-long-enough',
          before: null,
          after: body.draft,
    ...
    if (body.action === 'confirm') {
      confirmed += 1
      return route.fulfill({ json: { result: { entityType: 'expense', entityId: 'e1' } } })
    }
    if (body.action === 'save_draft') {
      row.extraction = body.extraction
      row.revision += 1
  ...
  await expect(page.getByLabel('Quantity').last()).toHaveValue('2')
  await expect(page.getByLabel('Amount').last()).toHaveValue('18.50')
  ...
  await page.getByRole('button', { name: 'Preview expense' }).click()
  const expenseDialog = page.getByRole('dialog', { name: 'Confirm exact proposed expense' })
  await expect(expenseDialog).toBeVisible()
  await expect(expenseDialog).toContainText('Corrected Garage')
  await expect(expenseDialog).toContainText('120')
  ...
  const maintenanceDialog = page.getByRole('dialog', { name: 'Confirm exact proposed maintenance' })
  await expect(maintenanceDialog).toBeVisible()
  await maintenanceDialog.getByRole('button', { name: 'Confirm creation' }).click()
