...
test('document review is usable on mobile and cancellation does not confirm', async ({ page }) => {
  ...
  await page.getByLabel('Private PDF, JPEG, or PNG').setInputFiles({ name: 'invoice.pdf', mimeType: 'application/pdf', buffer: Buffer.from('%PDF-1.7\n%%EOF') })
  ...
  await expect(page.getByLabel('Private PDF, JPEG, or PNG')).toBeFocused()
