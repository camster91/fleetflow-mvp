...
test('authenticated intelligence brief supports the daily attention workflow at 375px', async ({ page, baseURL }) => {
  ...
  const finding = {
    ...
    actionUrl: '/deliveries?record=delivery-1',
    feedback: null,
  ...
  const secondFinding = {
    ...
    evidence: [
      { entityType: 'vehicle', entityId: 'vehicle-2', field: 'status', value: 'inspection overdue', timestamp: null },
    ],
  }
  ...
  await page.route(/\/api\/(vehicles|deliveries|maintenance|sop|clients)(\?.*)?$/, (route) =>
    route.fulfill({ json: emptyCollection })
  )
  ...
  await page.route('**/api/intelligence/brief', (route) => {
    const findings = [{ ...finding, feedback: savedFeedback }, secondFinding].filter(
  ...
  await page.route('**/api/intelligence/findings', async (route) => {
    ...
    if (request.method() === 'PATCH') {
      ...
      return route.fulfill({
        json: { finding: { ...finding, feedback: body.action === 'HELPFUL' ? 'HELPFUL' : null } },
      })
    }
  ...
  await expect(firstCard.getByRole('link', { name: 'Open delivery record' })).toHaveAttribute(
    'href',
    '/deliveries?record=delivery-1'
  )
  await expect(firstCard.getByRole('link', { name: 'Review delivery' })).toHaveAttribute(
    'href',
    '/deliveries?record=delivery-1'
  )
