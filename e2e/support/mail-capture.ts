...
interface CapturedEmail { to: string[]; subject: string; text: string; html: string; capturedAt: string }

async function messagesFor(address: string): Promise<CapturedEmail[]> {
  let files: string[]
  try { files = await readdir(E2E_EMAIL_CAPTURE_DIR) } catch { return [] }
  ...
  for (const file of files.filter((name) => name.endsWith('.json')).sort()) {
    ...
    } catch { /* a file still being written is picked up on the next poll */ }
  }
  return messages
}

/** Wait for the next captured message to `address` whose subject matches, newer than `since`. */
export async function waitForEmail(address: string, subject: RegExp, since: number, timeoutMs = 15_000): Promise<CapturedEmail> {
  const deadline = Date.now() + timeoutMs
  while (Date.now() < deadline) {
    const match = (await messagesFor(address)).reverse().find((message) => subject.test(message.subject) && Date.parse(message.capturedAt) >= since)
    if (match) return match
    await new Promise((resolve) => setTimeout(resolve, 200))
  }
  throw new Error(`No captured email to ${address} matching ${subject} (is the server running with E2E_EMAIL_CAPTURE_DIR=${E2E_EMAIL_CAPTURE_DIR}?)`)
}
