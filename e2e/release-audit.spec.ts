...
async function seriousAccessibilityViolations(page: Page) {
  ...
  const result = await page.evaluate(async () => (window as AxeWindow).axe.run(document, {
...
test('public entry points remain accessible and bounded at release viewports', async ({ page }) => {
  ...
  page.on('response', response => { if (response.status() === 401) unauthorizedPaths.push(new URL(response.url()).pathname) })
...
test('public internal links, login form, and PWA manifest are release-safe', async ({ page, request }, testInfo) => {
  ...
  await page.goto('/')
  const hrefs = await page.locator('a[href^="/"]').evaluateAll(links => [...new Set(links.map(link => link.getAttribute('href')).filter((href): href is string => Boolean(href)))])
  ...
  const manifest = await manifestResponse.json() as { name?: string; display?: string; icons?: Array<{ sizes?: string }> }
