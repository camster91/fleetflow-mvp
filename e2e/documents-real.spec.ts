...
test('real database upload, review, concurrent confirmation, isolation, and deletion', async ({ request, baseURL }) => {
  ...
  const maintenanceSave = await request.post(`${baseURL}/api/documents/${documentId}/extract`, { headers: { ...headers, 'content-type': 'application/json' }, data: { action: 'save_draft', revision, extraction: reviewed, draft: maintenanceDraft } }); expect(maintenanceSave.status()).toBe(200); revision = (await maintenanceSave.json()).revision
  const maintenancePreview = await request.post(`${baseURL}/api/documents/${documentId}/extract`, { headers: { ...headers, 'content-type': 'application/json' }, data: { action: 'preview', revision, draft: maintenanceDraft } }); expect(maintenancePreview.status()).toBe(200)
  const maintenanceConfirm = await request.post(`${baseURL}/api/documents/${documentId}/extract`, { headers: { ...headers, 'content-type': 'application/json' }, data: { action: 'confirm', confirm: true, previewToken: (await maintenancePreview.json()).previewToken } }); expect(maintenanceConfirm.status()).toBe(200); expect(await db.maintenanceTask.count({ where: { ownerId, title: 'Oil service' } })).toBe(1)
