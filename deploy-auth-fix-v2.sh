...
REMOTE_TMP="/tmp/fleetflow-auth-fix-v2"

echo "🚀 Starting FleetFlow Authentication Fix Deployment v2"
echo "====================================================="
...
mkdir -p lib pages/auth pages/api/auth
...
tar czf auth-fix.tar.gz lib pages
...
echo "Creating directories in container (as root)..."
docker exec -u 0 $CONTAINER_ID mkdir -p /app/lib /app/pages/auth /app/pages/api/auth
