...
export default function NotificationsPage() {
  ...
  const markRead = async (ids: string[]) => {
    await fetch('/api/notifications', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ notificationIds: ids }) });
  ...
  return (
    <DashboardLayout breadcrumbs={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Notifications' }]}>
        ...
        {(['all', 'unread'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
              filter === f ? 'bg-blue-900 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
            }`}>
      ...
      <Card>
        {isLoading ? <SkeletonTable rows={6} columns={1} /> : filtered.length === 0 ? (
          ...
          <div className="divide-y divide-slate-50">
            {filtered.map(n => (
              <div key={n.id} className={`flex gap-3 p-4 hover:bg-slate-50 transition-colors group ${ !n.read ? 'bg-blue-50/30' : '' }`}>
                <div className="mt-0.5 shrink-0">{typeIcon[n.type] ?? <Info className="h-5 w-5 text-slate-400" />}</div>
                <div className="flex-1 min-w-0" onClick={() => !n.read && markRead([n.id])} style={{ cursor: !n.read ? 'pointer' : 'default' }}>
                  ...
                  {!n.read && (
                    <button onClick={() => markRead([n.id])} className="mt-1.5 text-xs text-blue-600 hover:text-blue-700">
                      Mark as read
                    </button>
                  )}
                </div>
