  ...
  return (
    <DashboardLayout breadcrumbs={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'SOPs & Procedures' }]}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
        <StatCard title="Total Categories" value={categories.length} icon={<BookOpen className="h-6 w-6 text-blue-600" />} iconBgColor="bg-blue-50" />
        <StatCard title="Total Documents" value={categories.reduce((s, c) => s + c.count, 0)} icon={<FileText className="h-6 w-6 text-purple-600" />} iconBgColor="bg-purple-50" />
      </div>
      ...
      {isLoading ? <SkeletonTable rows={4} columns={3} /> : filtered.length === 0 ? (
        <Card>
          ...
          {filtered.map((cat) => (
            <Card key={cat.id} hover className="flex flex-col">
              ...
              <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-100">
                <span className="text-sm text-slate-500 flex items-center gap-1"><FileText className="h-4 w-4" />{cat.count} documents</span>
                {cat.lastUpdated && <span className="text-xs text-slate-400">{new Date(cat.lastUpdated).toLocaleDateString()}</span>}
              </div>
            </Card>
          ))}
        </div>
