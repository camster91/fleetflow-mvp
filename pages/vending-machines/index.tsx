...
export default function VendingMachinesPage() {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: machines.filter((m) => m.status === 'out-of-service').length,
    ...
    <form onSubmit={handleSubmit} className="px-6 py-4 space-y-3">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select
            value={form.type}
            onChange={(e) => setForm({ ...form, type: e.target.value as VendingMachine['type'] })}
            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
          >
        ...
        <div>
          ...
          <select
            value={form.status}
            onChange={(e) => setForm({ ...form, status: e.target.value as VendingMachine['status'] })}
            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
          >
            <option value="active">Active</option>
            <option value="maintenance">Maintenance</option>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button
          type="submit"
          disabled={saving}
          className="flex-1 px-4 py-2 bg-blue-900 text-white rounded-lg text-sm font-medium disabled:opacity-50"
        >
          {saving ? 'Saving...' : 'Save'}
        </button>
      </div>
