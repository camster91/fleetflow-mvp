...
import { Building, Plus, Search, Star, Phone, Mail, MapPin, Edit, ExternalLink } from 'lucide-react'
  ...
  return (
    <div className="flex">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          key={i}
          className={`h-3.5 w-3.5 ${i <= (value ?? 0) ? 'text-amber-400 fill-amber-400' : 'text-slate-200'}`}
        />
      ))}
    </div>
  )
...
function TypeBadge({ type }: { type?: string }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${map[type ?? 'other'] ?? map.other}`}>{label}</span>
  )
}
...
export default function ClientsPage() {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: summary?.restaurantHotel ?? 0,
      icon: <Building className="h-6 w-6 text-purple-600" />,
      iconBgColor: 'bg-purple-50',
    },
    {
      title: 'High Rating',
      value: summary?.highRating ?? 0,
