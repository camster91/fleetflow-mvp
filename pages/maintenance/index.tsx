    ...
    <DashboardLayout breadcrumbs={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Maintenance' }]}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex border border-slate-200 rounded-lg overflow-hidden">
              <button
                ...
                className={`px-3 py-1.5 text-sm flex items-center gap-1.5 ${viewMode === 'calendar' ? 'bg-blue-900 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 text-sm flex items-center gap-1.5 border-l border-slate-200 ${viewMode === 'list' ? 'bg-blue-900 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}
              >
            ...
            <Button
              ...
              onClick={() => {
                api
                  .getAllMatching<MaintenanceTask>('/api/maintenance', { ...list.queryParams, today: todayKey })
                  .then((rows) =>
                    downloadCSV(
                      'maintenance-tasks',
                      rows.map((t) => ({
                  ...
                  .catch((err: unknown) =>
                    toast.error(err instanceof Error ? err.message : 'Failed to export maintenance tasks')
                  )
              }}
            >
              Export CSV
      ...
      {viewMode === 'list' ? (
        ...
        <Card>
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search by task or vehicle..."
          ...
          {list.loading && filteredTasks.length === 0 ? (
            <SkeletonTable rows={5} columns={5} />
          ) : filteredTasks.length === 0 ? (
            <EmptyState
              type={isFiltered ? 'search' : 'data'}
              title="No tasks"
              description={isFiltered ? 'No maintenance tasks match your search or filter' : 'No maintenance tasks yet'}
            />
          ) : (
            <>
                          ...
                          <td className="py-3 px-4">
                            <div className="flex gap-1 justify-end">
                              <button
                                ...
                                className="px-2 py-1 text-xs bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg"
                              >
                                {canManage ? 'Edit' : 'View'}
                              </button>
                              {canManage && !task.completed && (
                                <button
                                  ...
                                  className="px-2 py-1 text-xs bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg"
                                >
                                  Complete
                                </button>
                              )}
              ...
              <div className="grid grid-cols-7 gap-px bg-slate-200 border border-slate-200 rounded-lg overflow-hidden min-w-[640px]">
                ...
                {days.map((day) => {
                  ...
                  return (
                    <div
                      key={day}
                      className={`bg-white p-2 min-h-[100px] ${isToday ? 'ring-2 ring-blue-500 ring-inset' : ''}`}
                    >
