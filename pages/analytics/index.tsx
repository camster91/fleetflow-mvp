      ...
      >
        <div className="mb-3">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Transparent rules rank recorded operational indicators. Scores are not failure predictions.
          </p>
        </div>
        {isLoading ? (
          <div role="status" className="h-20 animate-pulse rounded-xl bg-slate-100">
            <span className="sr-only">Loading maintenance attention scores</span>
          </div>
        ) : (data?.maintenanceRisk?.items.length ?? 0) === 0 ? (
          <p className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-600">
            No vehicles are available to assess.
          </p>
        ) : (
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            {data?.maintenanceRisk?.items.map((risk) => (
              <MaintenanceRiskBadge key={risk.vehicleId} risk={risk} />
            ))}
          </div>
        )}
        {data?.maintenanceRisk &&
          (!data.maintenanceRisk.coverage.vehiclesComplete || !data.maintenanceRisk.coverage.tasksComplete) && (
            <p role="note" className="mt-3 text-sm text-amber-800">
              This view reached its bounded record limit. Scores cover only the evaluated records shown.
            </p>
          )}
      </section>
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {[
                {
                  label: 'Delivered',
                  count: s?.deliveries?.delivered ?? 0,
                  color: 'bg-emerald-500',
                  pct: deliveryTotal ? Math.round(((s?.deliveries?.delivered ?? 0) / deliveryTotal) * 100) : 0,
                },
                {
                  label: 'In Transit',
                  count: s?.deliveries?.inTransit ?? 0,
                  color: 'bg-blue-500',
                  pct: deliveryTotal ? Math.round(((s?.deliveries?.inTransit ?? 0) / deliveryTotal) * 100) : 0,
                },
                {
                  label: 'Pending',
                  count: s?.deliveries?.pending ?? 0,
                  color: 'bg-amber-400',
                  pct: deliveryTotal ? Math.round(((s?.deliveries?.pending ?? 0) / deliveryTotal) * 100) : 0,
                },
                {
                  label: 'Delayed',
                  count: s?.deliveries?.delayed ?? 0,
                  color: 'bg-red-500',
                  pct: deliveryTotal ? Math.round(((s?.deliveries?.delayed ?? 0) / deliveryTotal) * 100) : 0,
                },
