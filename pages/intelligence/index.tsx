      ...
      try {
        if (!cursor && !capabilitiesLoaded.current) {
          ...
          if (capabilityResponse.ok) {
            const brief = (await capabilityResponse.json()) as {
              capabilities?: { manage?: boolean; feedback?: boolean }
            }
            setCapabilities({
              manage: brief.capabilities?.manage === true,
              feedback: brief.capabilities?.feedback === true,
            })
          }
        ...
        if (!nextController.signal.aborted && !(loadError instanceof DOMException && loadError.name === 'AbortError'))
          cursor ? setLoadMoreError(true) : setError(true)
      } finally {
  ...
  const mutate = async (finding: BriefFinding, action: 'HELPFUL' | 'NOT_HELPFUL' | 'DISMISS' | 'RESOLVE') => {
    ...
    setFindings((current) =>
      remove
        ...
        : current.map((item) => (item.id === finding.id ? { ...item, feedback: action } : item))
    )
    try {
        ...
        if (remove)
          requestAnimationFrame(() =>
            (nextId
              ...
              : document.getElementById(findings.length === 1 ? 'intelligence-empty' : 'intelligence-heading')
            )?.focus()
          )
      }
    } catch {
      if (filterGeneration.current === generation && operationVersions.current.get(finding.id) === version) {
        setFindings((current) => {
          ...
          return exists
            ...
            : [...current.slice(0, Math.max(0, index)), finding, ...current.slice(Math.max(0, index))]
        })
      ...
      if (
        mutationsInFlight.current.size === 0 &&
        refetchAfterMutations.current &&
        filterGeneration.current === generation
      ) {
  ...
  return (
    <DashboardLayout breadcrumbs={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Fleet intelligence' }]}>
      <div className="mx-auto max-w-4xl min-w-0 space-y-5">
        <div>
          <h1 id="intelligence-heading" tabIndex={-1} className="text-2xl font-bold text-slate-950">
            Fleet intelligence findings
          </h1>
          <p className="mt-1 text-sm text-slate-600">
            Review the record-backed issues Fleetvera has prioritized for this workspace.
          </p>
        </div>
        ...
        {loading && (
          <div
            role="status"
            aria-label="Loading intelligence findings"
            className="h-40 animate-pulse rounded-xl bg-slate-100"
          >
            <span className="sr-only">Loading intelligence findings</span>
          </div>
        )}
        {error && (
          <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-900">
            ...
            <p className="text-sm">No data was changed.</p>
            <button type="button" onClick={() => void load()} className="mt-2 min-h-11 px-2 font-medium underline">
              Try again
            </button>
          </div>
        )}
        ...
        {loadMoreError && (
          <div role="alert" className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
            Could not load more findings. Existing findings are still shown.{' '}
            <button
              type="button"
              onClick={() => nextCursor && void load(nextCursor)}
              className="min-h-11 px-2 font-medium underline"
            >
              Retry load more
            </button>
          </div>
        )}
        {nextCursor && !error && !loadMoreError && (
          <button
            type="button"
            disabled={loadingMore}
            onClick={() => void load(nextCursor)}
            className="min-h-11 rounded-lg border border-slate-300 bg-white px-4 py-2 font-medium text-slate-800 disabled:opacity-50"
          >
