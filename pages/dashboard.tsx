...
} from '@/components/role-dashboards/CommandCentre'
...
export default function Dashboard() {
  ...
  const [totals, setTotals] = useState<Record<'vehicles' | 'deliveries' | 'maintenance', number | null>>({
    vehicles: null,
    deliveries: null,
    maintenance: null,
  })
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRole(resolveDashboardRole(rawRole))
      setDecisions(body.decisions)
      setActions(body.actions)
      setOnboardingCompleted(body.onboardingCompleted !== false)
      setData({
        vehicles: sources.vehicles.items as unknown as CommandCentreData['vehicles'],
        deliveries: sources.deliveries.items as unknown as CommandCentreData['deliveries'],
        maintenance: sources.maintenance.items as unknown as CommandCentreData['maintenance'],
      })
      setTotals({
        vehicles: sources.vehicles?.total ?? null,
        deliveries: sources.deliveries?.total ?? null,
        maintenance: sources.maintenance?.total ?? null,
      })
      const nextAvailability = {
        vehicles: sources.vehicles?.available === true,
        deliveries: sources.deliveries?.available === true,
        maintenance: sources.maintenance?.available === true,
      }
      // A source the role may not view is not a partial outage.
      const nextRestricted = {
        vehicles: sources.vehicles?.error === 'FORBIDDEN',
        deliveries: sources.deliveries?.error === 'FORBIDDEN',
        maintenance: sources.maintenance?.error === 'FORBIDDEN',
      }
      setAvailability(nextAvailability)
      setRestricted(nextRestricted)
      setPartial(
        (Object.keys(nextAvailability) as Array<keyof typeof nextAvailability>).some(
          (key) => !nextAvailability[key] && !nextRestricted[key]
        )
      )
      setLoading(false)
    } catch (error) {
  ...
  }, [cancelActiveRequest, load, status])
  useEffect(() => {
    ...
    void fetch('/api/pilot/events', {
      ...
      body: JSON.stringify({ eventType: 'DASHBOARD_OPENED', sessionKey }),
    }).catch(() => undefined)
  }, [role])
  if ((loading && !role) || status === 'loading')
    return (
      <DashboardLayout>
  ...
  if (status !== 'authenticated' || error || !role)
    return (
      <DashboardLayout>
        <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-5">
          <h1 className="font-semibold">Unable to open this workspace dashboard</h1>
          <p className="mt-1 text-sm">Your workspace access could not be verified.</p>
          {status === 'authenticated' && (
            <button onClick={load} className="mt-4 min-h-[44px] rounded-lg border px-4">
              Retry
            </button>
          )}
        </div>
  ...
  return (
    <DashboardLayout>
      {role === 'admin' ? (
        ...
        <MaintenanceDashboard {...props} />
      ) : role === 'driver' ? (
        <DriverDashboard {...props} />
      ) : (
        <ViewerDashboard {...props} />
      )}
      {role === 'admin' && (
        <OnboardingModal
          isOpen={!onboardingCompleted && !onboardingDismissed}
