  ...
  try {
    ...
    if (isSetup) {
      ...
      if (
        ...
        !backupCodes.every(
          (backupCode: unknown) => typeof backupCode === 'string' && BACKUP_CODE_PATTERN.test(backupCode)
        )
      ) {
        return res.status(400).json({
      ...
      if (
        ...
        !backupCodes.every(
          (backupCode: string, index: number) =>
            typeof storedBackupCodes[index] === 'string' && bcrypt.compareSync(backupCode, storedBackupCodes[index])
        )
      ) {
        return res.status(400).json({
          error: '2FA setup data is invalid or stale',
      ...
      try {
        await sendBackupCodesEmail(user.email, user.name || 'there', backupCodes)
      } catch (emailError) {
