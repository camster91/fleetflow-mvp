...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const vehicle = await prisma.vehicle.findFirst({
    where: { AND: [{ id }, tenant.resourceWhere, ...(isDriverRole(tenant.role)?[{assignedDriverId:session.user.id}]:[])] },
  });
  ...
  return res.json(isDriverRole(tenant.role)?{vehicle:driverVehicleDto(vehicle),maintenanceTasks:maintenanceTasks.map(task=>({id:task.id,title:task.title,type:task.type,dueDate:task.dueDate,priority:task.priority,completed:task.completed,completedDate:task.completedDate})),driverUser:null}:{vehicle,maintenanceTasks,driverUser});
}
