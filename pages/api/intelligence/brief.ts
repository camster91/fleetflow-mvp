...
const briefSelect = {
  ...
  action: true, actionUrl: true, status: true, feedback: true, generatedAt: true, expiresAt: true, resolvedAt: true,
} as const
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { rows, totalOpen, run } = await prisma.$transaction(async tx => {
      const [findings, count, latestRun] = await Promise.all([
        tx.intelligenceFinding.findMany({ where: openWhere, select: briefSelect, orderBy: [{ score: 'desc' }, { id: 'asc' }], take: MAX_BRIEF_FINDINGS }),
    ...
    }, { isolationLevel: 'RepeatableRead', maxWait: BRIEF_TRANSACTION_MAX_WAIT_MS, timeout: BRIEF_TRANSACTION_TIMEOUT_MS })
    ...
    const reasons = !run ? ['NEVER_GENERATED'] : [
