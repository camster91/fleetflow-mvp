...
const requestSchema = z
  ...
  .superRefine((value, ctx) => {
    if (value.actionTaken && !value.outcomeCategory)
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['outcomeCategory'], message: 'Select an operational outcome' })
    if (!value.actionTaken && value.outcomeCategory)
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['outcomeCategory'], message: 'Outcome requires an action' })
  })
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const unique = {
    submittedById_scopeKey_idempotencyKey: { submittedById: context.session.user.id, scopeKey, idempotencyKey },
  }
  ...
  if (replay)
    return replay.requestHash === requestHash
      ? res.status(200).json({ feedback: presentFeedback(replay), replayed: true })
      : res.status(409).json({ error: 'Idempotency-Key was already used for different feedback' })
  ...
  if (!quota.allowed) {
    ...
    return res.status(429).json({ error: 'Too many pilot feedback requests' })
  }
  const now = new Date()
  try {
    ...
    const risk = scoreMaintenanceRisk(
      {
        vehicle,
        tasks: taskRows.slice(0, TASK_LIMIT),
        serviceMileage: null,
        currency: process.env.MAINTENANCE_RISK_CURRENCY,
        costUnit: 'major',
        sourceComplete: taskRows.length <= TASK_LIMIT,
      },
      { now }
    )
    const expiresAt = new Date(now.getTime() + 180 * 86_400_000)
    const row = await prisma.$transaction(async (tx) => {
      const feedback = await tx.maintenanceRiskFeedback.create({
        data: {
          ownerId: context.tenant.ownerId,
          teamId: context.tenant.teamId,
          vehicleId: vehicle.id,
          rubricVersion: risk.rubricVersion,
          scoreSnapshot: risk.score,
          bandSnapshot: risk.band,
          sourceComplete: risk.sourceComplete,
          completenessPercent: risk.completeness.percent,
      ...
      await tx.auditLog.create({
        data: {
          ...
          description: 'Recorded consented maintenance attention pilot feedback',
...
export function feedbackRequestHash(value: z.infer<typeof requestSchema>) {
  const canonical = JSON.stringify({
    ...
    outcomeCategory: value.outcomeCategory ?? null,
...
async function withdrawFeedback(
  ...
  const scopeKey = context.tenant.teamId ? `team:${context.tenant.teamId}` : `owner:${context.tenant.ownerId}`
  const row = await prisma.maintenanceRiskFeedback.findFirst({
    where: {
      id,
      submittedById: context.session.user.id,
      scopeKey,
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.auditLog.create({
      data: {
        ...
        userName: context.session.user.name ?? null,
        userRole: context.tenant.role,
        action: 'pilot_feedback_withdrawn',
        entityType: 'maintenance_risk',
        entityId: row.id,
        description: 'Withdrew maintenance attention pilot feedback',
        metadata: JSON.stringify({ vehicleId: row.vehicleId, rubricVersion: row.rubricVersion }),
      },
    })
  })
