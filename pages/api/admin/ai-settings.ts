...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (parsed.data.enabled && !providerReadiness(provider, modelVersion, process.env).ready)
    return res.status(400).json({ error: 'Configured AI provider is not ready' })
  if ((existing?.configVersion ?? 0) !== parsed.data.expectedConfigVersion)
    return res.status(409).json({ error: 'AI settings changed; reload and retry' })
  if (
    existing &&
    existing.enabled === parsed.data.enabled &&
    existing.killSwitch === parsed.data.killSwitch &&
    existing.retentionDays === parsed.data.retentionDays
  )
    return res.status(200).json(publicConfig(existing))
  const nextVersion = parsed.data.expectedConfigVersion + 1,
    { expectedConfigVersion: _, ...settings } = parsed.data
  try {
    const saved = await prisma.$transaction(async (tx) => {
      if (existing) {
        const changed = await tx.aiWorkspaceConfig.updateMany({
          where: { scopeKey: key, configVersion: parsed.data.expectedConfigVersion },
          data: {
            ...settings,
            provider,
            modelVersion,
        ...
        if (changed.count !== 1) throw Object.assign(new Error('stale'), { code: 'STALE_CONFIG' })
      } else
        await tx.aiWorkspaceConfig.create({
          data: {
            scopeKey: key,
      ...
      await tx.aiControlAudit.create({
        data: {
          scopeKey: key,
    ...
    return res.status(200).json(publicConfig(saved))
  } catch (error) {
    if ((error as { code?: string }).code === 'STALE_CONFIG' || (error as { code?: string }).code === 'P2002')
      return res.status(409).json({ error: 'AI settings changed; reload and retry' })
    throw error
  }
}
