...
const confirmRequest = z
  .object({ action: z.literal('confirm'), confirm: z.literal(true), previewToken: z.string().min(20).max(12_000) })
  .strict()
const editRequest = z
  .object({
...
const storage = () => {
  ...
  if (!configured && process.env.NODE_ENV === 'production') throw new Error('DOCUMENT_STORAGE_PATH is not configured')
...
async function load(id: string, ownerId: string, teamId: string | null) {
  return prisma.documentUpload.findFirst({
    where: { id, ownerId, teamId, deletedAt: null, expiresAt: { gt: new Date() } },
  })
}

async function confirmWrite(
  payload: ReturnType<typeof verifyDocumentConfirmation>,
  ...
  try {
    return await prisma.$transaction(
      async (tx) => {
        ...
        const doc = await tx.documentUpload.findFirst({
          where: {
        ...
        if (
          ...
          !doc.extraction ||
          !doc.reviewDraft ||
          createHash('sha256').update(doc.extraction).digest('hex') !== payload.extractionDigest ||
          doc.reviewDraft !== JSON.stringify(payload.draft)
        )
          throw Object.assign(new Error('Reviewed draft changed since preview'), { statusCode: 409 })
        const vehicle = await tx.vehicle.findFirst({
          where: {
            id: payload.draft.vehicleId,
            ...(payload.teamId ? { teamId: payload.teamId } : { ownerId: payload.ownerId, teamId: null }),
          },
        })
        ...
        if (payload.draft.kind === 'maintenance') {
          ...
          const row = await tx.maintenanceTask.create({
            data: {
              title: values.type,
              type: values.type,
        ...
        await tx.documentExecution.create({
          data: {
            documentId: doc.id,
            nonce: payload.nonce,
            confirmerId: user.id,
            confirmerSnapshot: user.name || user.id,
        ...
        await tx.auditLog.create({
          data: {
            ...
            userRole: role,
            action: 'document_draft_confirmed',
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!parsed.success) return res.status(400).json({ error: 'Invalid document operation' })
  ...
  try {
    if (parsed.data.action === 'extract') {
      ...
      const processingToken = randomBytes(18).toString('base64url')
      const claimed = await prisma.documentUpload.updateMany({
        where: {
          id: doc.id,
          revision: doc.revision,
          OR: [
            { status: { in: ['UPLOADED', 'FAILED', 'EXTRACTED'] } },
            { status: 'EXTRACTING', processingLeaseUntil: { lte: claimNow } },
          ],
          deletedAt: null,
        ...
        data: {
          status: 'EXTRACTING',
          processingToken,
          processingLeaseUntil: new Date(claimNow.getTime() + 5_000),
      ...
      const renewal = setInterval(() => {
        void prisma.documentUpload
          .updateMany({
            ...
            data: { processingLeaseUntil: new Date(Date.now() + 5_000) },
          })
          .then((result) => {
      ...
      try {
        ...
        const inspected = inspectDocument(bytes, doc.mimeType, {
          authoritativeComplexPdf: doc.scanStatus === 'CLEAN_AUTHORITATIVE',
        })
        const extraction = await runDocumentExtraction(
          extractor(),
          { bytes, mime: doc.mimeType, pages: inspected.pages },
          { timeoutMs: 20_000, signal: controller.signal }
        )
        const finalizeNow = new Date()
        const updated = await prisma.documentUpload.updateMany({
          where: {
            id: doc.id,
            status: 'EXTRACTING',
            revision: claimRevision,
            ...
            storageKey: doc.storageKey,
            deletedAt: null,
            expiresAt: { gt: finalizeNow },
          },
          data: {
            extraction: JSON.stringify(extraction),
            status: 'EXTRACTED',
            extractedAt: finalizeNow,
        ...
        if (updated.count !== 1) {
          controller.abort()
          return res.status(409).json({ error: 'Extraction ownership changed; result was discarded' })
        }
        return res.status(200).json({ extraction, revision: claimRevision + 1 })
      } catch (error) {
        const failed = await prisma.documentUpload.updateMany({
          where: {
            id: doc.id,
            status: 'EXTRACTING',
            revision: claimRevision,
          ...
          data: { status: 'FAILED', processingToken: null, processingLeaseUntil: null, revision: { increment: 1 } },
        })
        if (failed.count !== 1) {
          controller.abort()
          return res.status(409).json({ error: 'Extraction ownership changed; failure was discarded' })
        }
        throw error
      } finally {
    ...
    if (parsed.data.action === 'save_draft') {
      const updated = await prisma.documentUpload.updateMany({
        ...
        data: {
          extraction: JSON.stringify(parsed.data.extraction),
          reviewDraft: parsed.data.draft ? JSON.stringify(parsed.data.draft) : null,
          status: 'REVIEWED',
          revision: { increment: 1 },
        },
      })
    ...
    if (parsed.data.action === 'preview') {
      if (
        doc.revision !== parsed.data.revision ||
        doc.status !== 'REVIEWED' ||
        !doc.extraction ||
        !doc.reviewDraft ||
        doc.reviewDraft !== JSON.stringify(parsed.data.draft)
      )
        return res.status(409).json({ error: 'Save this exact reviewed draft before previewing it' })
      const vehicle = await prisma.vehicle.findFirst({
        where: { id: parsed.data.draft.vehicleId, AND: [context.tenant.resourceWhere] },
      ...
      const token = signDocumentConfirmation(
        {
          ...
          extractionDigest: createHash('sha256').update(doc.extraction).digest('hex'),
          revision: doc.revision,
          draft: parsed.data.draft,
        },
        { secret: ring.currentSecret, kid: ring.currentKid, nonce: randomBytes(18).toString('base64url') }
      )
      return res.status(200).json({
        previewToken: token,
        before: null,
        after: parsed.data.draft,
        warning: 'No fleet record changes until you confirm.',
      })
    }
    const payload = verifyDocumentConfirmation(parsed.data.previewToken, { secrets: keys().secrets })
    if (
      payload.documentId !== doc.id ||
      payload.ownerId !== context.tenant.ownerId ||
      payload.teamId !== context.tenant.teamId ||
      payload.proposerId !== context.session.user.id
    )
      return res.status(403).json({ error: 'Confirmation does not belong to this reviewer and workspace' })
    ...
    return res
      .status(status)
      .json({ error: status === 503 ? 'Document processing is unavailable' : (error as Error).message })
  }
}
