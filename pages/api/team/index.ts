  ...
  if (req.method === 'PUT') {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        await tx.teamMember.update({ where: { id: memberId }, data: { role } })
        if (ctx.member.role === 'DRIVER' && role !== 'DRIVER')
          await clearDriverAssignments(tx, ctx.member.teamId, ctx.member.userId, {
            actorId: userId,
    ...
    try {
      await prisma.$transaction(async (tx) => {
        await clearDriverAssignments(tx, ctx.member.teamId, ctx.member.userId, {
          actorId: userId,
          actorName: session.user.name,
          actorRole: ctx.isOwner ? 'OWNER' : ctx.userMembership?.role,
        })
