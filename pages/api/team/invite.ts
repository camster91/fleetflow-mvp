...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const assignerRole: TeamRole | undefined =
      team.ownerId === session.user.id
        ? 'OWNER'
        : (team.members.find((member) => member.userId === session.user.id && member.status === 'ACCEPTED')?.role as
            TeamRole | undefined)
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const existingMembers = await tx.teamMember.findMany({
        where: {
          teamId,
          OR: [{ inviteeEmail: { in: normalizedEmails } }, { userId: { in: existingUsers.map((u) => u.id) } }],
        },
      })
      ...
      const seatsToReserve = actions.filter((action) => action.kind === 'create' || action.kind === 'reactivate').length
    ...
    const inviterName = (session.user as { name?: string | null }).name || session.user.email || 'A team member'
