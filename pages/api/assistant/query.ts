...
const requestSchema = z
  .object({ question: z.string().trim().min(1).max(500), selectedEntity: selectedEntitySchema.optional() })
  .strict()
...
function evidenceFindings(claims: AssistantClaim[], sources: AssistantSource[]): SafeFinding[] {
  return sources.slice(0, 20).map((item, index) => {
    const evidence = claims
      .filter((entry) => entry.citationIds.includes(item.id))
    ...
    return {
      ...
      severity: 'medium',
      title: item.label,
      explanation: evidence || 'This authorized record supports the requested fleet summary.',
      recommendedAction: `Review ${item.type} record`,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!plan.supported)
    return res.status(422).json({
      error:
        'I can answer questions about fleet priorities, due maintenance, recorded maintenance costs, delivery exceptions, specific vehicles or clients, and activity in a stated date range.',
      code: plan.reason,
    })
  try {
    const result = await timeout(
      runFleetTool(
        plan,
        {
          resourceWhere: context.tenant.resourceWhere,
          findingScope: { ownerId: context.tenant.ownerId, teamId: context.tenant.teamId },
        },
        {}
      )
    )
    if (!result.claims.length)
      return res.status(200).json({
        mode: 'deterministic',
        empty: true,
        answer: { claims: [], summary: 'No matching Fleetvera records were found.' },
        sources: [],
      })
    ...
    if (!runtime.enabled) {
      await recordWorkspaceAiTelemetry(
        {
          provider: runtime.config.provider,
          modelVersion: runtime.config.modelVersion,
          status: 'fallback',
          errorCode: runtime.reason === 'provider_misconfigured' ? 'provider_misconfigured' : 'provider_disabled',
          latencyMs: 0,
          inputTokens: null,
          outputTokens: null,
        },
      ...
      return res.status(200).json({
        mode: 'deterministic',
        empty: false,
        degradationReason: runtime.reason,
        answer: { claims: result.claims },
        sources,
      })
    }
    const providerDeadline = new AbortController()
    const providerTimer = setTimeout(() => providerDeadline.abort(), 7_500)
    providerTimer.unref?.()
    let generated
    try {
      generated = await generateFleetSummary(
        { findings: evidenceFindings(result.claims, sources) },
        {
          env: {
            ...process.env,
            AI_PROVIDER: runtime.config.provider,
            OPENAI_MODEL: runtime.config.modelVersion,
            AI_TIMEOUT_MS: process.env.AI_TIMEOUT_MS || '7000',
          },
    ...
    await recordWorkspaceAiTelemetry(
      {
        provider: generated.meta.provider ?? runtime.config.provider,
        modelVersion: generated.meta.model ?? runtime.config.modelVersion,
        status: generated.meta.status,
        ...(generated.meta.fallbackReason ? { errorCode: generated.meta.fallbackReason } : {}),
        latencyMs: generated.meta.latencyMs ?? 0,
        inputTokens: generated.meta.usage?.inputTokens ?? null,
        outputTokens: generated.meta.usage?.outputTokens ?? null,
      },
      context.tenant
    ).catch(() => undefined)
