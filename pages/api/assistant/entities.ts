...
const querySchema = z.object({
  type: z.enum(['vehicle', 'delivery', 'client']),
  ...
  try {
    if (parsed.data.type === 'vehicle') {
      const rows = await prisma.vehicle.findMany({
        where: parsed.data.q
          ? { AND: [context.tenant.resourceWhere, { name: { contains: parsed.data.q, mode: 'insensitive' } }] }
    ...
    if (parsed.data.type === 'delivery') {
      const rows = await prisma.delivery.findMany({
        where: parsed.data.q
          ? { AND: [context.tenant.resourceWhere, { customer: { contains: parsed.data.q, mode: 'insensitive' } }] }
          : context.tenant.resourceWhere,
        select: { id: true, customer: true },
        orderBy: [{ customer: 'asc' }, { id: 'asc' }],
        take: 20,
      })
    ...
    const rows = await prisma.client.findMany({
      where: parsed.data.q
        ? {
            AND: [
              ...
              {
                OR: [
                  { businessName: { contains: parsed.data.q, mode: 'insensitive' } },
      ...
      orderBy: [{ businessName: 'asc' }, { name: 'asc' }, { id: 'asc' }],
      take: 20,
    })
    return res
      .status(200)
      .json({ entities: rows.map((row) => ({ id: row.id, label: (row.businessName || row.name).slice(0, 120) })) })
  } catch {
    console.error('Assistant selector failed')
    return res.status(503).json({ error: 'Records could not be loaded' })
  }
}
