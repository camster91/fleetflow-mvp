...
function canUse(
  action: SuggestedAction,
  role: Parameters<typeof canViewBusinessData>[0],
) {
  if (action.type === "create_maintenance_task")
    return canManageMaintenance(role);
  if (action.type === "update_delivery_status")
    return canManageDeliveries(role);
...
async function loadBefore(action: SuggestedAction, resourceWhere: object) {
  ...
  if (action.type === "draft_weekly_summary") {
    const [vehicles, openMaintenance, activeDeliveries, completedDeliveries] =
      await Promise.all([
...
async function execute(
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      if (payload.sourceFindingId && !(await validSourceFinding(tenant.ownerId, tenant.teamId, payload.sourceFindingId, action, tx)))
      ...
      if (action.type === "update_delivery_status") {
        ...
        const transition = applyDeliveryStatusTransition(
          current,
          values,
        ...
        const updated = await tx.delivery.updateMany({
          where: {
            AND: [
              { id: action.deliveryId, updatedAt: current.updatedAt },
        ...
        result = {
          ...
          after: {
            ...transition.fields,
            completedTime:
              transition.fields.completedTime?.toISOString() ?? null,
          },
        ...
        if (!payload.generatedEntityId)
          throw new Error("Missing generated entity ID");
        const task = await tx.maintenanceTask.create({
          data: {
            ...
            partsNeeded: values.partsNeeded
              ? JSON.stringify(values.partsNeeded)
              : null,
...
export default async function handler(
  ...
  if (
    !(await rateLimitMiddleware(
      ...
      res,
      "api",
      `assistant-actions:${context.session.user.id}`,
    ))
  )
    return;
  ...
  if (confirmation.success) {
    ...
    try {
      ...
      return res
        .status(String(error).includes("expired") ? 410 : 400)
        .json({
          error: String(error).includes("expired")
    ...
    if (!canUse(payload.action, context.tenant.role))
    ...
    if (
      ...
      return res
        .status(409)
        .json({ error: "Source finding is no longer available" });
    try {
      return res
        .status(200)
        .json({ executed: true, result: await execute(payload, context) });
    } catch (error) {
      if (
        ...
        return res
          ...
          .json({
            error:
              "This action was already confirmed or conflicted with another update",
          });
      const status =
  ...
  if (!parsed.success)
    return res.status(400).json({ error: "Invalid suggested action" });
  const { action, sourceFindingId } = parsed.data;
  if (!canUse(action, context.tenant.role))
    return res.status(403).json({ error: "Insufficient permissions" });
  const isWrite =
    action.type === "create_maintenance_task" ||
    action.type === "update_delivery_status";
  ...
  if (
    action.type === "update_delivery_status" &&
    before.updatedAt !== action.expectedUpdatedAt
  )
    return res.status(409).json({ error: "Record changed since suggestion" });
  if (
    action.type === "create_maintenance_task" &&
    before.vehicleUpdatedAt !== action.expectedVehicleUpdatedAt
  )
    return res.status(409).json({ error: "Record changed since suggestion" });
  if (action.type === "open_edit" || action.type === "draft_weekly_summary") {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        metadata: JSON.stringify({
          sourceSuggestion,
          previewNonce: verifyActionPreview(issued.token, { secrets: keyRing.secrets })
            .jti,
          actionType: action.type,
        }),
      },
  ...
  try {
    ...
    return res
      .status(503)
      .json({ error: "Suggested actions are not configured" });
  }
}
