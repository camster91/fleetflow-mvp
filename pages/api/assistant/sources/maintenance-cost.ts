...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const vehicle = await prisma.vehicle.findFirst({
      where: { AND: [context.tenant.resourceWhere, { id: parsed.data }] },
      select: { id: true },
    })
    ...
    return res.status(200).json({
      ...
      total: aggregate._sum.actualCost ?? 0,
      count,
      contributors: contributors.map((item) => ({
        id: item.id,
        actualCost: item.actualCost,
        href: `/maintenance?record=${encodeURIComponent(item.id)}`,
      })),
      contributorsTruncated: count > contributors.length,
    })
  } catch {
    console.error('Maintenance cost source failed')
    return res.status(503).json({ error: 'Source could not be loaded' })
  }
}
