...
import { Key, Plus, Copy, Trash2, Webhook, Clock, AlertTriangle, Check } from 'lucide-react'
...
export default function APISettingsPage() {
  ...
  const generateKey = async () => {
    ...
    try {
      ...
      setApiKeys((current) => [
        { ...createdKey, key: MASKED_API_KEY, scopes: createdKey.scopes || ['read'] },
        ...current,
      ])
  ...
  const revokeKey = async (id: string) => {
    const ok = await confirmAction('Apps using this key will stop working immediately.', 'Revoke API key')
    if (!ok) return
    setRevokingId(id)
    try {
  ...
  return (
    <DashboardLayout
      breadcrumbs={[{ label: 'Dashboard', href: '/' }, { label: 'Settings', href: '/settings' }, { label: 'API' }]}
    >
      <PageHeader title="API & Integrations" subtitle="Manage API keys and webhook configurations" />
      ...
      <Card className="mb-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500">Use these keys to access the Fleetvera API programmatically</p>
          </div>
          <Button variant="primary" onClick={() => setShowKeyModal(true)} iconLeft={<Plus className="h-4 w-4" />}>
            Generate Key
          </Button>
        </div>

        {loading ? (
          <p className="py-8 text-center text-slate-500">Loading API keys...</p>
        ) : apiKeys.length === 0 ? (
          ...
          <div className="space-y-4">
            {apiKeys.map((apiKey) => (
              <div key={apiKey.id} className="flex items-center justify-between p-4 border border-slate-200 rounded-lg">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm text-slate-500">
                    ...
                    {apiKey.lastUsedAt && <span>Last used {formatDistanceToNow(new Date(apiKey.lastUsedAt))} ago</span>}
                  </div>
                </div>
                <Button
                  variant="ghost"
        ...
        <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-amber-700 mt-1">
              Keep your API keys secure. Do not share them in public repositories or client-side code. If a key is
      ...
      <Card>
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500">Receive real-time notifications when events occur in your account</p>
          </div>
          <Button variant="outline" disabled iconLeft={<Plus className="h-4 w-4" />}>
            ...
            {webhooks.map((webhook) => (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">Events: {webhook.events.join(', ')}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <Modal isOpen={showKeyModal} onClose={() => setShowKeyModal(false)} title="Generate API Key" size="sm">
        <div className="space-y-4">
          ...
          <Button variant="primary" fullWidth disabled={!keyName.trim()} loading={generating} onClick={generateKey}>
            Generate secure key
          </Button>
        </div>
      </Modal>
      <Modal isOpen={!!showNewKey} onClose={() => setShowNewKey(null)} title="API Key Generated" size="md">
        <div className="text-center">
          <div className="w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <Check className="h-8 w-8 text-emerald-600" />
          </div>
          <h3 className="text-lg font-semibold text-slate-900 mb-2">Your New API Key</h3>
          <p className="text-sm text-slate-600 mb-4">Copy this key now. You won't be able to see it again!</p>
