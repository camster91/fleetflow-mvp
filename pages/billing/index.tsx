...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/subscription/status').then((r) =>
        r.ok ? r.json() : Promise.reject(r.status === 403 ? 'forbidden' : undefined)
      ),
  ...
  const savings =
    pricing && pricing.monthly.amount > 0
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        {notBillingViewer && (
          ...
          >
            ...
            <div>
              <p className="font-semibold">Fleetvera is free during the beta</p>
              <p>No payment details are needed. Billing is managed by your workspace owner or admin.</p>
            </div>
          </div>
        )}
        ...
        {!notBillingViewer && role !== null && !canManage && !billingCheckFailed && (
          <p className="text-sm text-slate-600">
            Billing is managed by your workspace owner or admin. You can view the plan but not change it.
          </p>
        )}
        ...
        {billingCheckFailed && (
          ...
          >
            ...
            <div>
              <p className="font-semibold">Billing status could not be verified</p>
        ...
        {!notBillingViewer && !billingAvailable && !billingCheckFailed && !hasPaidSubscription && (
          ...
          >
            ...
            <div>
              <p className="font-semibold">Fleetvera is free during the beta</p>
              <p>No payment details are needed. We will give beta workspaces advance notice before paid plans start.</p>
            </div>
          </div>
        )}
        ...
        {sub && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            ...
            <div className="space-y-2 text-sm text-slate-600">
              <p>
                <span className="font-medium text-slate-800">Plan:</span> Fleetvera Pro
              </p>
              {sub.status === 'TRIAL' && (
                <p>
              ...
              {sub.currentPeriodEnd && (
                <p>
                  <span className="font-medium text-slate-800">
                    {sub.cancelAtPeriodEnd ? 'Access through:' : 'Renews:'}
                  </span>{' '}
              ...
              {sub.cancelAtPeriodEnd && (
                <p className="text-amber-600">
                  ...
                  {sub.currentPeriodEnd ? ` on ${new Date(sub.currentPeriodEnd).toLocaleDateString()}` : ''}.
                </p>
              )}
            </div>
            ...
            <a
              ...
              className="mt-4 ml-3 inline-flex items-center gap-2 px-4 py-2 text-sm text-blue-800 hover:underline"
            >
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-6">
              <div className="bg-slate-50 rounded-xl p-5">
                <p className="text-2xl font-bold text-slate-900">
                  {pricing ? formatPrice(pricing.monthly.amount, pricing.monthly.currency) : 'Unavailable'}
              ...
              <div className="bg-blue-50 rounded-xl p-5 ring-2 ring-blue-200">
                <p className="text-2xl font-bold text-slate-900">
                  {pricing ? formatPrice(pricing.yearly.amount, pricing.yearly.currency) : 'Unavailable'}
                  <span className="text-base text-slate-500 font-normal">/yr</span>
                </p>
