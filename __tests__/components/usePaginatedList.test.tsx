...
describe('readListState', () => {
  it('reads valid URL state and falls back to defaults', () => {
    ...
    expect(
      readListState(
        { page: '3', pageSize: '50', q: 'van', status: 'delayed', sort: 'name', order: 'desc' },
        ['status'],
        ['name']
      )
    ...
    expect(
      readListState(
        { page: '-2', pageSize: '1000', sort: 'ownerId', order: 'desc', status: 'all' },
        ['status'],
        ['name']
      )
...
describe('usePaginatedList', () => {
  const setup = (fetchPage = jest.fn().mockResolvedValue(page([{ id: 'a' }], 1, { total: 7 }))) => {
    const hook = renderHook(() =>
      usePaginatedList<{ id: string }, { total: number }>({
        fetchPage,
        filterKeys: ['status'],
        sortKeys: ['name'],
  ...
  it('pushes page, page-size, filter and sort changes to the URL and resets to page 1 on filter changes', async () => {
    ...
    expect(mockRouter.push).toHaveBeenLastCalledWith(
      { pathname: '/vehicles', query: { page: '4', record: 'keep-me' } },
      undefined,
      { shallow: true }
    )
    ...
    expect(mockRouter.push).toHaveBeenLastCalledWith(
      { pathname: '/vehicles', query: { status: 'active', record: 'keep-me' } },
      undefined,
      { shallow: true }
    )
  ...
  it('refetches when the URL changes (e.g. the back button)', async () => {
    ...
    rerender()
    await waitFor(() =>
      expect(fetchPage).toHaveBeenLastCalledWith(expect.objectContaining({ page: 2 }), expect.any(AbortSignal))
    )
  ...
  it('debounces search input into a single URL replace and resets the page', async () => {
    ...
    act(() => {
      result.current.setSearchInput('v')
    })
    act(() => {
      result.current.setSearchInput('va')
    })
    act(() => {
      result.current.setSearchInput('van ')
    })
    ...
    expect(mockRouter.replace).toHaveBeenCalledWith({ pathname: '/vehicles', query: { q: 'van' } }, undefined, {
      shallow: true,
    })
    rerender()
  ...
  it('moves to the last page when the requested page is past the end', async () => {
    ...
    setup(fetchPage)
    await waitFor(() =>
      expect(mockRouter.replace).toHaveBeenCalledWith({ pathname: '/vehicles', query: { page: '2' } }, undefined, {
        shallow: true,
      })
    )
  })
  ...
  it('reports errors, keeps only the newest response and supports background refetch', async () => {
    ...
    await act(async () => {
      await result.current.refetch({ background: true })
    })
