...
describe('delivery mutation hardening', () => {
  ...
  it('deletes the delivery and records the audit event in one transaction', () => {
    const deleteTransaction = itemRoute.indexOf(
      'await prisma.$transaction(async (tx) => {',
      itemRoute.indexOf("if (req.method === 'DELETE')")
    )
    expect(deleteTransaction).toBeGreaterThan(-1)
    expect(itemRoute.indexOf('await tx.delivery.delete({ where: { id } })', deleteTransaction)).toBeGreaterThan(
      deleteTransaction
    )
