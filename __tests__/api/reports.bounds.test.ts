...
const readRoute = (name: string) => readFileSync(
  join(process.cwd(), `pages/api/reports/${name}.ts`),
  'utf8',
)

describe('report route resource controls', () => {
  it.each(['deliveries', 'maintenance'])(
    'bounds %s analytics by date and row count',
    (name) => {
  ...
  it.each(['deliveries', 'maintenance', 'fleet', 'export'])(
    ...
    (name) => {
      expect(readRoute(name)).toContain(
        "rateLimitMiddleware(req, res, 'api', `reports:${session.user.id}`)",
      )
    },
  )
  ...
  it('caps fleet materialization and keeps exports on the shared policy', () => {
    ...
    expect(readRoute('export')).toContain(
      'parseReportDateRange(req.query.startDate, req.query.endDate)',
    )
