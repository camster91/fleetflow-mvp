...
jest.mock('@/lib/prisma', () => ({ prisma: {
  team: { findMany: jest.fn() }, invoice: { findMany: jest.fn() },
} }))
...
describe('GET /api/subscription/invoices', () => {
  ...
  it('returns an empty tenant-scoped invoice list', async () => {
    ...
    expect(prisma.invoice.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: { userId: 'owner-1' }, take: 100 }))
  })
  ...
  it('uses the selected team owner for an authorized manager', async () => {
    ...
    ;(prisma.team.findMany as jest.Mock).mockResolvedValue([{ id: 'team-1', ownerId: 'owner-1', members: [{ role: 'MANAGER' }] }])
    const { req, res } = createMocks<NextApiRequest, NextApiResponse>({ method: 'GET', headers: { 'x-team-id': 'team-1' } })
    await handler(req, res)
    expect(res._getStatusCode()).toBe(200)
  ...
  it('denies a viewer access to invoice history', async () => {
    ...
    ;(prisma.team.findMany as jest.Mock).mockResolvedValue([{ id: 'team-1', ownerId: 'owner-1', members: [{ role: 'VIEWER' }] }])
    const { req, res } = createMocks<NextApiRequest, NextApiResponse>({ method: 'GET', headers: { 'x-team-id': 'team-1' } })
    await handler(req, res)
  ...
  it('returns paid and failed invoice records without Stripe identifiers', async () => {
    ;(prisma.invoice.findMany as jest.Mock).mockResolvedValue([
      { id: 'i1', amount: 4900, currency: 'USD', status: 'paid', invoicePdf: 'https://invoice.stripe.com/i1.pdf', createdAt: new Date(), periodStart: new Date(), periodEnd: new Date() },
      { id: 'i2', amount: 0, currency: 'USD', status: 'failed', invoicePdf: null, createdAt: new Date(), periodStart: new Date(), periodEnd: new Date() },
    ])
    const { req, res } = request()
    await handler(req, res)
    expect(res._getStatusCode()).toBe(200)
  ...
  it('omits malformed records and removes unsafe PDF links', async () => {
    ;(prisma.invoice.findMany as jest.Mock).mockResolvedValue([
      { id: 'safe', amount: -500, currency: 'cad', status: 'paid', invoicePdf: 'javascript:alert(1)', createdAt: new Date(), periodStart: new Date(), periodEnd: new Date() },
      { id: 'bad-currency', amount: 10, currency: 'ZZZ', status: 'paid', invoicePdf: null, createdAt: new Date(), periodStart: new Date(), periodEnd: new Date() },
      { id: 'bad-date', amount: 10, currency: 'USD', status: 'paid', invoicePdf: null, createdAt: new Date('invalid'), periodStart: new Date(), periodEnd: new Date() },
    ])
    const { req, res } = request()
    await handler(req, res)
    expect(res._getStatusCode()).toBe(200)
    expect(res._getJSONData()).toEqual({ invoices: [expect.objectContaining({ id: 'safe', amount: -500, currency: 'CAD', invoicePdf: null })] })
  })
