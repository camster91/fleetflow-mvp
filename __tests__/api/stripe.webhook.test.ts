...
describe('Stripe webhook compatibility', () => {
  ...
  test('acknowledges a duplicate event without applying it twice', async () => {
    ...
    ;(prisma.stripeWebhookEvent.create as jest.Mock).mockRejectedValue(
      Object.assign(new Error('duplicate'), {
        code: 'P2002',
        meta: { modelName: 'StripeWebhookEvent', target: ['id'] },
      })
    )
  ...
  test('returns 500 for a downstream unique collision so Stripe retries', async () => {
    ...
    ;(prisma.invoice.upsert as jest.Mock).mockRejectedValue(
      Object.assign(new Error('invoice collision'), {
        code: 'P2002',
        meta: { modelName: 'Invoice', target: ['stripeInvoiceId'] },
      })
    )
    const errorSpy = jest.spyOn(console, 'error').mockImplementation(() => undefined)
    const { req, res } = webhookRequest()
  ...
  test('passes the raw request bytes unchanged to signature validation', async () => {
    ...
    Object.assign(req, {
      async *[Symbol.asyncIterator]() {
        yield raw.subarray(0, 8)
        yield raw.subarray(8)
      },
    })
    await handler(req, res)
    expect(res._getStatusCode()).toBe(200)
  ...
  test('records a failed invoice and transitions the subscription to past due', async () => {
    ...
    expect(prisma.invoice.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ where: { stripeInvoiceId: 'in_123' } })
    )
    expect(prisma.invoice.upsert).toHaveBeenCalledWith(
  ...
  test('keeps authoritative paid invoice truth when a delayed payment_failed event arrives', async () => {
    ;(constructWebhookEvent as jest.Mock).mockReturnValue({
      id: 'evt_failed_but_paid',
      created: 300,
      type: 'invoice.payment_failed',
      data: { object: invoiceFixture('failed') },
    })
    ;(prisma.subscription.findUnique as jest.Mock).mockResolvedValue({
      id: 'local-sub',
      userId: 'owner-1',
      status: 'ACTIVE',
      stripeLastEventCreated: 200,
    })
    const { req, res } = webhookRequest()
    await handler(req, res)
    expect(res._getStatusCode()).toBe(200)
  ...
  test.each([
    ...
    'stores explicit minor units for %s including zero or credits',
    async (eventType, status, amountPaid, amountDue, expected) => {
      ...
      expect(prisma.invoice.upsert).toHaveBeenCalledWith(
        expect.objectContaining({
          create: expect.objectContaining({ amount: expected }),
          update: expect.objectContaining({ amount: expected }),
        })
      )
    }
  )
    ...
    expect(prisma.subscription.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          status: 'CANCELLED',
          stripeLastEventCreated: 250,
          stripeLastEventId: 'evt_cancelled',
        }),
      })
    )
    expect(prisma.auditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ action: 'status_changed', description: expect.stringContaining('CANCELLED') }),
      })
    )
  })
  ...
  test('does not let an older failed-payment event regress a paid subscription', async () => {
    ...
    ;(prisma.subscription.findUnique as jest.Mock).mockResolvedValue({
      ...
      status: 'ACTIVE',
      stripeLastEventCreated: 200,
      stripeLastEventId: 'evt_paid_new',
    })
    const { req, res } = webhookRequest()
    await handler(req, res)
    expect(res._getStatusCode()).toBe(200)
    expect(prisma.invoice.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ update: expect.objectContaining({ status: 'paid', amount: 4900 }) })
    )
    expect(prisma.subscription.update).not.toHaveBeenCalled()
  ...
  test('does not let an older paid invoice resurrect a deleted subscription', async () => {
    ...
    ;(prisma.subscription.findUnique as jest.Mock).mockResolvedValue({
      ...
      status: 'CANCELLED',
      stripeLastEventCreated: 200,
      stripeLastEventId: 'evt_deleted_new',
    })
    const { req, res } = webhookRequest()
    await handler(req, res)
    expect(res._getStatusCode()).toBe(200)
    expect(prisma.invoice.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ update: expect.objectContaining({ status: 'failed', amount: 4900 }) })
    )
    expect(prisma.subscription.update).not.toHaveBeenCalled()
  })
  ...
  test('does not let an older paid event override current failed invoice or past-due subscription truth', async () => {
    ...
    ;(prisma.subscription.findUnique as jest.Mock).mockResolvedValue({
      ...
      status: 'PAST_DUE',
      stripeLastEventCreated: 200,
      stripeLastEventId: 'evt_failed_new',
    })
    const { req, res } = webhookRequest()
    await handler(req, res)
    expect(res._getStatusCode()).toBe(200)
  ...
  test('does not let an older checkout resurrect a deleted subscription', async () => {
    ;(constructWebhookEvent as jest.Mock).mockReturnValue({
      ...
      data: {
        object: {
          metadata: { userId: 'owner-1', priceId: 'price_monthly' },
  ...
  test('uses authoritative Stripe state for equal-timestamp failed payment', async () => {
    ...
    ;(prisma.subscription.findUnique as jest.Mock).mockResolvedValue({
      ...
      status: 'ACTIVE',
      stripeLastEventCreated: 200,
      stripeLastEventId: 'evt_paid_same',
    })
    const { req, res } = webhookRequest()
    await handler(req, res)
    expect(res._getStatusCode()).toBe(200)
  ...
  test('uses authoritative Stripe state for equal-timestamp checkout', async () => {
    ...
    ;(prisma.subscription.findUnique as jest.Mock).mockResolvedValue({
      ...
      status: 'CANCELLED',
      stripeLastEventCreated: 200,
      stripeLastEventId: 'evt_deleted_same',
    })
