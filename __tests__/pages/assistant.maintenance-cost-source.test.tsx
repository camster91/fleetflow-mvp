...
describe('maintenance cost source page', () => {
  it('visibly substantiates the authoritative total/count and contributing links', async () => {
    ...
    expect(screen.getByText(/12 recorded maintenance entries/i)).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /maintenance record m1/i })).toHaveAttribute(
      'href',
      '/maintenance?record=m1'
    )
    expect(screen.getByText(/additional contributing records/i)).toBeInTheDocument()
  })
})
