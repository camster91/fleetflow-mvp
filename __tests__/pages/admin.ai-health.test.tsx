...
test('AI health page exposes accessible status, usage, fallback, errors, and evaluation landmarks', async () => {
  ...
  expect(await screen.findByRole('heading', { name: 'AI health' })).toBeInTheDocument()
  expect(screen.getByText('Enabled')).toBeInTheDocument()
  expect(screen.getByText(/fixtures@v1/)).toBeInTheDocument()
  expect(screen.getByRole('table', { name: 'AI errors' })).toBeInTheDocument()
  expect(screen.getByRole('checkbox', { name: 'Enable AI for this workspace' })).toBeChecked()
  expect(screen.getByRole('button', { name: 'Save AI controls' })).toBeInTheDocument()
  expect(screen.getByRole('link', { name: 'Configure transactional email' })).toHaveAttribute(
    'href',
