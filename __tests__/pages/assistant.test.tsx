...
describe('Ask Fleetvera page', () => {
  ...
  it('offers supported prompts and renders cited answers', async () => {
    ;(fetch as jest.Mock).mockResolvedValue({
      ...
      json: async () => ({
        empty: false,
        answer: { claims: [{ text: 'One delivery is late.', citationIds: ['delivery:d1'] }] },
        sources: [
  ...
  it('shows an aborted state when cancellation is clicked', async () => {
    ...
    render(<AssistantPage />)
    fireEvent.change(screen.getByLabelText(/ask a fleet question/i), {
      target: { value: 'What needs attention today?' },
    })
  ...
  it('supports a successful retry after a clear retryable error', async () => {
    ;(fetch as jest.Mock).mockRejectedValueOnce(new Error('offline')).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ empty: true, answer: { claims: [], summary: 'No matching records.' }, sources: [] }),
    })
    render(<AssistantPage />)
    fireEvent.change(screen.getByLabelText(/ask a fleet question/i), {
      target: { value: 'What needs attention today?' },
  ...
  it('loads and selects only authorized bounded records and /ask renders the same guarded experience', async () => {
    ;(fetch as jest.Mock).mockResolvedValue({
      ok: true,
      json: async () => ({ entities: [{ id: 'v1', label: 'Van 1' }] }),
    })
    const { unmount } = render(<AssistantPage />)
    expect(screen.getByLabelText(/record type/i)).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: /load authorized records/i }))
    expect(await screen.findByRole('option', { name: 'Van 1' })).toBeInTheDocument()
    fireEvent.change(screen.getByLabelText(/authorized record/i), { target: { value: 'v1' } })
    expect(screen.getByRole('button', { name: /summarize selected vehicle/i })).toBeEnabled()
    unmount()
    render(<AskAliasPage />)
    expect(screen.getByRole('heading', { name: /what would you like to check/i })).toBeInTheDocument()
  })
  ...
  it('prevents a cancelled stale request from overwriting an immediate retry', async () => {
    ...
    ;(fetch as jest.Mock)
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ empty: true, answer: { claims: [], summary: 'Fresh retry result.' }, sources: [] }),
      })
    render(<AssistantPage />)
    const input = screen.getByLabelText(/ask a fleet question/i)
    fireEvent.change(input, { target: { value: 'What needs attention today?' } })
    fireEvent.click(screen.getByRole('button', { name: /^ask$/i }))
    fireEvent.click(await screen.findByRole('button', { name: /cancel/i }))
    fireEvent.click(screen.getByRole('button', { name: /^ask$/i }))
    expect(await screen.findByText('Fresh retry result.')).toBeInTheDocument()
    resolveFirst({
      ok: true,
      json: async () => ({ empty: true, answer: { claims: [], summary: 'Stale cancelled result.' }, sources: [] }),
    })
    await Promise.resolve()
    expect(screen.queryByText('Stale cancelled result.')).not.toBeInTheDocument()
  })
})
