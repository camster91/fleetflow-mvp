...
jest.mock('@/lib/prisma', () => ({
  prisma: {
    ...
    maintenanceTask: {
      ...
      findMany: jest.fn(async () => [{ vehicleName: 'Van', type: 'Oil', dueDate: new Date('2026-12-01T00:00:00.000Z') }]),
...
jest.mock('@/lib/apiAuth', () => ({
  requireTenantContext: jest.fn(async () => ({
    session: { user: { id: 'u1' } },
    tenant: { ownerId: 'o1', teamId: 'team-1', role: 'OWNER', resourceWhere: { teamId: 'team-1' }, auditWhere: { teamId: 'team-1' } },
  })),
}))
...
describe('GET /api/analytics/dashboard workspace "today"', () => {
  beforeEach(() => {
    ...
    jest.useFakeTimers({ now: new Date('2026-12-02T07:30:00.000Z'), doNotFake: ['nextTick', 'setImmediate', 'queueMicrotask'] })
  ...
  it('counts overdue and upcoming against the workspace calendar day', async () => {
    ...
    expect(countWheres).toContainEqual({ teamId: 'team-1', completed: false, dueDate: { gte: today, lte: new Date('2026-12-15T00:00:00.000Z') } })
