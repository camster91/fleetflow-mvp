...
describe('/api/assistant/entities', () => {
  ...
  it('returns bounded authorized vehicle selector projections', async () => {
    ...
    const { req, res } = createMocks({ method: 'GET', query: { type: 'vehicle', q: 'van' } })
    await handler(req as never, res as never)
    expect(prisma.vehicle.findMany).toHaveBeenCalledWith({
      where: { AND: [resourceWhere, { name: { contains: 'van', mode: 'insensitive' } }] },
  ...
  it('returns only client identity labels and no contact fields', async () => {
    ;(prisma.client.findMany as jest.Mock).mockResolvedValue([
      { id: 'c1', name: 'Client 1', businessName: 'Business 1' },
    ])
    const { req, res } = createMocks({ method: 'GET', query: { type: 'client' } })
    await handler(req as never, res as never)
    expect(prisma.client.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: resourceWhere, select: { id: true, name: true, businessName: true }, take: 20 })
    )
    expect(res._getData()).not.toMatch(/email|phone|address/i)
  })
  it.each([
    { method: 'POST', query: {} },
    { method: 'GET', query: { type: 'driver' } },
    { method: 'GET', query: { type: 'vehicle', q: 'x'.repeat(101) } },
  ])('rejects unsupported selector requests %#', async (init) => {
    const { req, res } = createMocks(init as never)
    await handler(req as never, res as never)
