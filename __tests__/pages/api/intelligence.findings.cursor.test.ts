...
describe('finding signed keyset cursor', () => {
  ...
  it('returns a small score-desc/id-asc page and signs the final returned key', async () => {
    ;(prisma.intelligenceFinding.findMany as jest.Mock).mockResolvedValue([row('a', 500), row('b', 500), row('c', 490)])
    ...
    expect(prisma.intelligenceFinding.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          AND: [exactScope, { status: 'OPEN', OR: [{ expiresAt: null }, { expiresAt: { gt: expect.any(Date) } }] }],
        },
    ...
    const second = createMocks({
      method: 'GET',
      query: { limit: '2', status: 'OPEN', cursor: body.pagination.nextCursor },
    })
  ...
  it.each([
    ...
    [
      ...
      {
        ...context,
        tenant: { ...context.tenant, ownerId: 'owner-2', teamId: 'team-2' },
      },
      'OPEN',
    ],
