...
const mockVehicle = {
  id: 'v-1', name: 'Van 1', status: 'active', driver: 'John Driver',
  location: '123 Main St', eta: '2h', mileage: 45000,
  maintenanceDue: false, ownerId: 'user-1',
};
const mockTasks = [
...
describe('GET /api/vehicles/[id]/details', () => {
  ...
  it('includes driver user info when user found by name', async () => {
    ...
    (prisma.user.findFirst as jest.Mock).mockResolvedValue({
      name: 'John Driver', email: 'john@fleet.com', image: null,
    });
    const { req, res } = createMocks({ method: 'GET', query: { id: 'v-1' } });
    await handler(req as any, res as any);
    expect(res._getStatusCode()).toBe(200);
