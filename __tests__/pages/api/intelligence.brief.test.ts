...
function row(overrides: Record<string, unknown> = {}) {
  return {
    id: 'finding-1', type: 'delivery-late', severity: 'high', confidence: 0.9,
    score: 500, ruleVersion: 'v1', title: 'Late delivery', explanation: 'Delivery is late.',
    evidence: JSON.stringify({ items: [{ entityType: 'delivery', entityId: 'd-1', field: 'scheduledTime', value: 'late', timestamp: NOW.toISOString() }], total: 1, truncated: false }),
    action: 'Review delivery', actionUrl: '/deliveries?record=d-1', status: 'OPEN', feedback: null,
    generatedAt: NOW, expiresAt: new Date('2026-08-09T16:00:00.000Z'), resolvedAt: null,
    ...overrides,
  }
}
...
describe('/api/intelligence/brief', () => {
  ...
  beforeEach(() => {
    ...
    tx = {
      ...
      intelligenceRun: { findUnique: jest.fn().mockResolvedValue({ id: 'team:team-1', ...scope, generatedAt: NOW, sourceComplete: true, findingsComplete: true, reconciliationComplete: true, evidenceComplete: true, findingTotal: 7, sourceCounts: '{"vehicle":12,"delivery":12,"maintenance":12,"client":12}' }) },
    }
  ...
  it('returns only five current open exact-workspace findings in deterministic order', async () => {
    ...
    expect(tx.intelligenceFinding.findMany).toHaveBeenCalledWith(expect.objectContaining({
      where: { ...scope, status: 'OPEN', OR: [{ expiresAt: null }, { expiresAt: { gt: NOW } }] },
      orderBy: [{ score: 'desc' }, { id: 'asc' }], take: 5,
    }))
    ...
    expect(res._getJSONData()).toEqual(expect.objectContaining({
      totalOpen: 7, generatedAt: NOW.toISOString(), capabilities: { refresh: true, manage: true, feedback: true },
    }))
  })
  ...
  it('never exposes invalid evidence or unsafe action URLs and reports incomplete coverage', async () => {
    ...
    tx.intelligenceRun.findUnique.mockResolvedValue({ ...(await tx.intelligenceRun.findUnique()), sourceComplete: false, evidenceComplete: false })
    const { req, res } = createMocks({ method: 'GET' })
    await handler(req as never, res as never)
    ...
    expect(body.coverage).toEqual(expect.objectContaining({ complete: false, sourceTruncated: true, evidenceComplete: false }))
  })

  it('removes evidence fields that could contain personal contact data', async () => {
    tx.intelligenceFinding.findMany.mockResolvedValue([row({
      evidence: JSON.stringify({ items: [{ entityType: 'client', entityId: 'c-1', field: 'email', value: 'private@example.test', timestamp: null }], total: 1, truncated: false }),
    })])
    const { req, res } = createMocks({ method: 'GET' })
    await handler(req as never, res as never)
  ...
  it('uses the same redacted presentation envelope as the full findings route', async () => {
    const evidence = JSON.stringify({ items: [{ entityType: 'client', entityId: 'c-1', field: 'phone', value: '555-0100', timestamp: null }], total: 1, truncated: false })
  ...
  it('marks the brief stale from persisted run metadata and treats never-generated empty state as incomplete', async () => {
    tx.intelligenceRun.findUnique.mockResolvedValue({ ...(await tx.intelligenceRun.findUnique()), generatedAt: new Date('2026-08-07T15:59:59Z') })
    const { req, res } = createMocks({ method: 'GET' })
    await handler(req as never, res as never)
    ...
    expect(never.res._getJSONData()).toEqual(expect.objectContaining({ generatedAt: null, stale: true, coverage: expect.objectContaining({ complete: false, reason: 'NEVER_GENERATED' }) }))
  })
