import {
  buildMaintenanceRisks,
  MAINTENANCE_RISK_TASK_LIMIT,
  MAINTENANCE_RISK_VEHICLE_LIMIT,
} from '@/pages/api/analytics/dashboard'
...
describe('analytics maintenance-risk projection', () => {
  ...
  it('uses the canonical tenant scope, bounded reads, safe projections, and deterministic ranking', async () => {
    ...
    const result = await buildMaintenanceRisks(
      { vehicle, maintenanceTask } as never,
      scope,
    ...
    expect(maintenanceTask.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { AND: [scope, { vehicleId: { in: ['v-low', 'v-high'] } }] },
  ...
  it('reports truncated coverage and never reads tasks when there are no vehicles', async () => {
    ...
    const empty = await buildMaintenanceRisks(
      { vehicle, maintenanceTask } as never,
      { ownerId: 'o1', teamId: null },
      new Date('2026-08-08T00:00:00.000Z')
    )
    ...
    vehicle.findMany.mockResolvedValue(
      Array.from({ length: MAINTENANCE_RISK_VEHICLE_LIMIT + 1 }, (_, index) => ({
        id: `v${index}`,
        name: `V${index}`,
        year: 2020,
        mileage: 0,
    ...
    const bounded = await buildMaintenanceRisks(
      { vehicle, maintenanceTask } as never,
      { ownerId: 'o1', teamId: null },
      new Date('2026-08-08T00:00:00.000Z')
    )
  ...
  it('marks each returned score incomplete instead of silently accepting the global task cap', async () => {
    const vehicle = {
      findMany: jest
        .fn()
        .mockResolvedValue([
          { id: 'v1', name: 'Van', year: 2020, mileage: 0, lastService: null, maintenanceDue: false },
        ]),
    }
    const maintenanceTask = {
      findMany: jest.fn().mockResolvedValue(
        Array.from({ length: MAINTENANCE_RISK_TASK_LIMIT + 1 }, (_, index) => ({
          id: `t${index}`,
          vehicleId: 'v1',
          type: 'Repair',
    ...
    expect(result.items[0].missingData).toContain(
