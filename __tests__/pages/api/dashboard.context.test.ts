...
jest.mock('@/lib/prisma', () => ({
  prisma: {
    ...
    maintenanceTask: { findMany: jest.fn(), count: jest.fn() },
  },
}))
...
describe('/api/dashboard/context', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    for (const model of [prisma.vehicle, prisma.delivery, prisma.maintenanceTask]) {
  ...
  it('returns a bounded canonical role DTO without identity data', async () => {
    ;(requireTenantContext as jest.Mock).mockResolvedValue({
      session: { user: { id: 'u-secret', email: 'private@example.test' } },
      tenant: { ownerId: 'owner-secret', teamId: 'team-secret', role: 'TECHNICIAN', resourceWhere: {} },
    })
    ...
    expect(res._getJSONData()).toEqual(
      expect.objectContaining({
        ...
        decisions: expect.arrayContaining([expect.any(String)]),
  ...
  it('scopes every driver source by stable user id and tenant scope', async () => {
    ;(requireTenantContext as jest.Mock).mockResolvedValue({
      session: { user: { id: 'driver-1' } },
      tenant: { ownerId: 'o1', teamId: 't1', role: 'DRIVER', resourceWhere: { ownerId: 'o1', teamId: 't1' } },
    })
    const { req, res } = createMocks({ method: 'GET' })
    await handler(req as never, res as never)
    expect(prisma.vehicle.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { ownerId: 'o1', teamId: 't1', assignedDriverId: 'driver-1' }, take: 10 })
    )
    ...
    expect(prisma.maintenanceTask.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { ownerId: 'o1', teamId: 't1', vehicle: { assignedDriverId: 'driver-1' } },
        take: 10,
      })
    )
  })
  it('returns only minimized operational fields for non-empty driver sources', async () => {
    ...
    ;(prisma.delivery.findMany as jest.Mock).mockResolvedValue([
      {
        ...
        contactPerson: 'SENTINEL_SECRET',
        accessCodes: 'SENTINEL_SECRET',
        securityNotes: 'SENTINEL_SECRET',
        photos: ['SENTINEL_SECRET'],
        notes: 'SENTINEL_SECRET',
        specialRequirements: 'SENTINEL_SECRET',
        internal: 'SENTINEL_SECRET',
        ownerId: 'o1',
        teamId: 't1',
      },
    ])
    ...
    expect(body.sources.deliveries.items[0]).toEqual(
      expect.objectContaining({ id: 'd1', address: '10 Main', assignedDriverId: 'driver-1' })
    )
    expect(body.sources.vehicles.items[0]).toEqual(
      expect.objectContaining({ id: 'v1', name: 'Truck 1', assignedDriverId: 'driver-1' })
    )
    expect(body.sources.maintenance.items[0]).toEqual(
      expect.objectContaining({ id: 'm1', vehicle: 'Truck 1', vehicleId: 'v1' })
    )
  ...
  it('reports authoritative totals beyond the bounded item window', async () => {
    ;(requireTenantContext as jest.Mock).mockResolvedValue({
      session: { user: { id: 'u1' } },
      tenant: { ownerId: 'o1', teamId: null, role: 'OWNER', resourceWhere: { ownerId: 'o1', teamId: null } },
    })
    ;(prisma.vehicle.count as jest.Mock).mockResolvedValue(73)
    const { req, res } = createMocks({ method: 'GET' })
    await handler(req as never, res as never)
    expect(res._getJSONData().sources.vehicles).toEqual(
      expect.objectContaining({ total: 73, truncated: true, items: [] })
    )
  })
  it('returns a sanitized service-unavailable response when every source fails', async () => {
    ...
    for (const model of [prisma.vehicle, prisma.delivery, prisma.maintenanceTask])
      (model.findMany as jest.Mock).mockRejectedValue(new Error('private database detail'))
    const { req, res } = createMocks({ method: 'GET' })
    await handler(req as never, res as never)
    expect(res._getStatusCode()).toBe(503)
    expect(res._getJSONData()).toEqual({ error: 'Dashboard data unavailable' })
    expect(res._getData()).not.toContain('private')
  })
  ...
  it.each([
    ...
    ;(requireTenantContext as jest.Mock).mockResolvedValue({
      session: { user: { id: 'u1' } },
      tenant: { ownerId: 'o1', teamId: 't1', role, resourceWhere: { teamId: 't1' } },
    })
    ;(prisma.vehicle.findMany as jest.Mock).mockResolvedValue([
      { id: 'v1', name: 'Van SENTINEL_VEHICLE', status: 'ACTIVE' },
    ])
    ;(prisma.delivery.findMany as jest.Mock).mockResolvedValue([
      { id: 'd1', customer: 'SENTINEL_CUSTOMER', address: '1 Main', status: 'PENDING' },
    ])
    ;(prisma.maintenanceTask.findMany as jest.Mock).mockResolvedValue([
      {
        id: 'm1',
  ...
  it('keeps a restricted role dashboard up when its permitted sources load, and 503s only when they all fail', async () => {
    ;(requireTenantContext as jest.Mock).mockResolvedValue({
      session: { user: { id: 'u1' } },
      tenant: { ownerId: 'o1', teamId: 't1', role: 'TECHNICIAN', resourceWhere: { teamId: 't1' } },
    })
    let call = createMocks({ method: 'GET' })
    await handler(call.req as never, call.res as never)
    expect(call.res._getStatusCode()).toBe(200)
    ;(prisma.maintenanceTask.findMany as jest.Mock).mockRejectedValue(new Error('down'))
    call = createMocks({ method: 'GET' })
    await handler(call.req as never, call.res as never)
