...
jest.mock('@/lib/apiAuth', () => ({
  requireTenantContext: jest.fn(async () => ({
    session: { user: { id: 'admin', email: 'admin@test.invalid', name: 'Admin' } },
...
const active = () => ({
  ...
  connection: { scopeKey: 'team:team-a', provider: 'quickbooks', generation: 2, revokedAt: null, status: 'CONNECTING' },
})

function request(query: Record<string, string> = {}) {
  return createMocks({
    method: 'GET',
    query: { provider: 'quickbooks', code: 'code-a', state, realmId: 'realm-a', ...query },
  })
}
...
describe('QuickBooks OAuth callback lifecycle', () => {
  ...
  it('consumes a valid state once and stores only an encrypted credential envelope', async () => {
    ...
    expect(mockPrisma.integrationOAuthState.updateMany).toHaveBeenCalledWith(
  ...
  it('classifies invalid_grant as reconnect-required and clears unusable credentials', async () => {
    ...
    expect(mockPrisma.integrationConnection.updateMany).toHaveBeenLastCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ id: 'connection-a', generation: 2 }),
