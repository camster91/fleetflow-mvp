...
const finding = (id = 'f1', entityType = 'delivery', entityId = 'd1') => ({
  id,
  status: 'OPEN',
  resolvedAt: null,
  expiresAt: null,
...
describe('/api/assistant/actions/execute', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    process.env.ACTION_PREVIEW_KEYS = JSON.stringify({
      current: 'test-secret-that-is-at-least-32-bytes-long',
      previous: 'previous-secret-that-is-at-least-32-bytes',
    })
  ...
  it('revalidates finding state inside the write transaction', async () => {
    ...
    const tx = {
      intelligenceFinding: { findFirst: jest.fn().mockResolvedValue({ ...finding(), status: 'DISMISSED' }) },
      delivery: { findFirst: jest.fn(), updateMany: jest.fn() },
    ...
    expect(tx.actionExecution.create).not.toHaveBeenCalled()
  ...
  it('keeps weekly summaries and edit prefills non-mutating with signed audited provenance', async () => {
    ...
    ;(prisma.delivery.count as jest.Mock).mockResolvedValueOnce(3).mockResolvedValueOnce(4)
    let mocks = createMocks({
      method: 'POST',
  ...
  ])('shows exact allowlisted current and proposed %s edit fields', async (entityType, current, values, href) => {
