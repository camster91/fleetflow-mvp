...
jest.mock('@/lib/apiRateLimit', () => ({
  consumePublicApiQuota: jest.fn(async () => ({ allowed: true, remaining: 19, retryAfter: 3600 })),
}))
jest.mock('@/lib/prisma', () => ({
  prisma: {
    vehicle: { findFirst: jest.fn() },
    maintenanceTask: { findMany: jest.fn() },
    maintenanceRiskFeedback: { findUnique: jest.fn(), findFirst: jest.fn() },
    $transaction: jest.fn(),
  },
}))
...
const context = {
  session: { user: { id: 'member-1', name: 'Pilot User', role: 'MEMBER' } },
  tenant: { ownerId: 'owner-1', teamId: 'team-1', role: 'MEMBER', resourceWhere: scope },
}
...
describe('POST /api/intelligence/maintenance-risk-feedback', () => {
  beforeEach(() => {
    ...
    ;(prisma.vehicle.findFirst as jest.Mock).mockResolvedValue({
      ...
      year: 2020,
      mileage: 50000,
      lastService: new Date('2026-01-01T00:00:00.000Z'),
  ...
  it('requires explicit research consent and bounded structured input', async () => {
    const { req, res } = createMocks({
      method: 'POST',
      headers: { 'idempotency-key': 'pilot_request_12345678' },
      body: { vehicleId: 'v1', helpful: true, actionTaken: false, consent: false },
    })
    ...
    const invalid = createMocks({
      ...
      body: {
        ...
        helpful: true,
        actionTaken: true,
        outcomeCategory: 'BREAKDOWN_PREDICTED',
        notes: 'x'.repeat(501),
        consent: true,
      },
    })
    ...
    const contradictory = createMocks({
      method: 'POST',
      headers: { 'idempotency-key': 'pilot_request_12345678' },
      body: { vehicleId: 'v1', helpful: true, actionTaken: false, outcomeCategory: 'SERVICE_COMPLETED', consent: true },
    })
  ...
  it('recomputes the score from a canonically scoped vehicle and bounded tasks, then audits consented feedback', async () => {
    ...
    ;(prisma.$transaction as jest.Mock).mockImplementation(async (fn: Function) =>
      fn({ maintenanceRiskFeedback: { create }, auditLog: { create: audit } })
    )
    const { req, res } = createMocks({
      method: 'POST',
      headers: { 'idempotency-key': 'pilot_request_12345678' },
    ...
    expect(create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        ...
        idempotencyKey: 'pilot_request_12345678',
  ...
  it('rejects hostile/missing production origin before auth and requires idempotency before body work', async () => {
    ...
    let mock = createMocks({ method: 'POST', body: {} })
    await handler(mock.req as never, mock.res as never)
    expect(requireTenantContext).not.toHaveBeenCalled()
    ;(assertSameOrigin as jest.Mock).mockReturnValue(true)
    mock = createMocks({ method: 'POST', body: { vehicleId: 'v1', helpful: true, actionTaken: false, consent: true } })
    await handler(mock.req as never, mock.res as never)
    expect(mock.res._getStatusCode()).toBe(400)
    expect(prisma.vehicle.findFirst).not.toHaveBeenCalled()
  })

  it('returns a durable replay before scoring and rate limits before vehicle reads', async () => {
    ;(prisma.maintenanceRiskFeedback.findUnique as jest.Mock).mockResolvedValueOnce({
      id: 'same',
      requestHash: feedbackRequestHash({ vehicleId: 'v1', helpful: true, actionTaken: false, consent: true }),
      rubricVersion: 'maintenance-risk-v1',
      scoreSnapshot: 1,
      bandSnapshot: 'low',
      createdAt: new Date(),
    })
    let mock = createMocks({
      method: 'POST',
      headers: { 'idempotency-key': 'pilot_request_12345678' },
      body: { vehicleId: 'v1', helpful: true, actionTaken: false, consent: true },
    })
    await handler(mock.req as never, mock.res as never)
    ...
    ;(prisma.maintenanceRiskFeedback.findUnique as jest.Mock).mockResolvedValue(null)
    ;(consumePublicApiQuota as jest.Mock).mockResolvedValueOnce({ allowed: false, remaining: 0, retryAfter: 60 })
    mock = createMocks({
      method: 'POST',
      headers: { 'idempotency-key': 'pilot_request_87654321' },
      body: { vehicleId: 'v1', helpful: true, actionTaken: false, consent: true },
    })
    await handler(mock.req as never, mock.res as never)
    expect(mock.res._getStatusCode()).toBe(429)
    expect(prisma.vehicle.findFirst).not.toHaveBeenCalled()
  })

  it('withdraws only the submitting user scoped feedback and audits the deletion', async () => {
    ;(prisma.maintenanceRiskFeedback.findFirst as jest.Mock).mockResolvedValue({
      id: 'f1',
      vehicleId: 'v1',
      rubricVersion: 'maintenance-risk-v1',
    })
    ...
    ;(prisma.$transaction as jest.Mock).mockImplementationOnce(async (fn: Function) =>
      fn({ maintenanceRiskFeedback: { delete: remove }, auditLog: { create: audit } })
    )
    const { req, res } = createMocks({ method: 'DELETE', body: { id: 'f1' } })
    await handler(req as never, res as never)
    expect(res._getStatusCode()).toBe(200)
    expect(prisma.maintenanceRiskFeedback.findFirst).toHaveBeenCalledWith({
      where: { id: 'f1', submittedById: 'member-1', scopeKey: 'team:team-1', ownerId: 'owner-1', teamId: 'team-1' },
    })
    expect(remove).toHaveBeenCalledWith({ where: { id: 'f1' } })
    expect(audit).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: 'pilot_feedback_withdrawn', entityId: 'f1' }),
    })
  })
  ...
  it('allows a demoted viewer to withdraw only their own exact workspace feedback', async () => {
    ...
    ;(prisma.maintenanceRiskFeedback.findFirst as jest.Mock).mockResolvedValueOnce({
      id: 'f1',
      vehicleId: 'v1',
      rubricVersion: 'maintenance-risk-v1',
    })
    ;(prisma.$transaction as jest.Mock).mockImplementationOnce(async (fn: Function) =>
      fn({ maintenanceRiskFeedback: { delete: jest.fn() }, auditLog: { create: jest.fn() } })
    )
    const mock = createMocks({ method: 'DELETE', body: { id: 'f1' } })
    await handler(mock.req as never, mock.res as never)
    expect(mock.res._getStatusCode()).toBe(200)
    expect(prisma.maintenanceRiskFeedback.findFirst).toHaveBeenCalledWith({
      where: { id: 'f1', submittedById: 'member-1', scopeKey: 'team:team-1', ownerId: 'owner-1', teamId: 'team-1' },
  ...
  it('returns 409 when the same idempotency key is reused with altered feedback', async () => {
    ...
    const mock = createMocks({
      method: 'POST',
      headers: { 'idempotency-key': 'pilot_request_12345678' },
      body: { vehicleId: 'changed', helpful: false, actionTaken: false, consent: true },
    })
    await handler(mock.req as never, mock.res as never)
    expect(mock.res._getStatusCode()).toBe(409)
    expect(prisma.vehicle.findFirst).not.toHaveBeenCalled()
    expect(prisma.$transaction).not.toHaveBeenCalled()
  })
  ...
  it('rejects cross-tenant vehicles, viewers, unsupported methods, and missing auth', async () => {
    ...
    let mock = createMocks({
      method: 'POST',
      headers: { 'idempotency-key': 'pilot_request_outside' },
      body: { vehicleId: 'outside', helpful: false, actionTaken: false, consent: true },
    })
    await handler(mock.req as never, mock.res as never)
    ...
    mock = createMocks({
      method: 'POST',
      headers: { 'idempotency-key': 'pilot_request_viewer' },
      body: { vehicleId: 'v1', helpful: false, actionTaken: false, consent: true },
    })
    await handler(mock.req as never, mock.res as never)
    expect(mock.res._getStatusCode()).toBe(403)
    mock = createMocks({ method: 'GET' })
    await handler(mock.req as never, mock.res as never)
    expect(mock.res._getStatusCode()).toBe(405)
    ;(prisma.vehicle.findFirst as jest.Mock).mockClear()
    ;(requireTenantContext as jest.Mock).mockResolvedValueOnce(null)
    mock = createMocks({ method: 'POST', body: {} })
    await handler(mock.req as never, mock.res as never)
    expect(prisma.vehicle.findFirst).not.toHaveBeenCalled()
  })
})
