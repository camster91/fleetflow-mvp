...
describe('finding regeneration scale', () => {
  ...
  it('uses one serializable snapshot and persists the ranked top 500 with O(1) reads and writes', async () => {
    ...
    tx.delivery.findMany.mockResolvedValue(
      Array.from({ length: FINDING_SOURCE_LIMIT }, (_, index) => ({
        id: `d-${index}`, status: 'pending', driver: null, vehicleId: null,
        scheduledTime: null, estimatedArrival: null, contactPerson: null,
    ...
    for (const model of [tx.vehicle, tx.delivery, tx.maintenanceTask, tx.client]) {
      ...
      expect(model.findMany).toHaveBeenCalledWith(expect.objectContaining({
        where: resourceWhere, take: FINDING_SOURCE_LIMIT + 1,
      }))
      expect(model.findMany.mock.invocationCallOrder[0])
        .toBeGreaterThan((prisma.$transaction as jest.Mock).mock.invocationCallOrder[0])
    }
    expect(tx.intelligenceFinding.findMany).toHaveBeenCalledTimes(1)
    expect(tx.intelligenceFinding.findMany.mock.invocationCallOrder[0])
      .toBeGreaterThan((prisma.$transaction as jest.Mock).mock.invocationCallOrder[0])
  ...
  it('updates 500 heterogeneous existing findings with one parameterized bulk write', async () => {
    ...
    ;(prisma.$transaction as jest.Mock).mockImplementation(async (callback: (value: typeof firstTx) => unknown) => callback(firstTx))
    ...
    secondTx.delivery.findMany.mockResolvedValue(firstTx.delivery.findMany.mock.calls.length
      ? await firstTx.delivery.findMany.mock.results[0].value : [])
    secondTx.intelligenceFinding.findMany.mockResolvedValue(
      created.map((entry: Record<string, unknown>) => ({
        id: entry.id, ownerId: entry.ownerId, teamId: entry.teamId, type: entry.type,
        status: entry.status, feedback: entry.feedback, ruleVersion: entry.ruleVersion,
        expiresAt: entry.expiresAt, resolvedAt: entry.resolvedAt,
      }))
    )
    secondTx.$executeRaw.mockResolvedValue(FINDING_PERSIST_LIMIT)
    ;(prisma.$transaction as jest.Mock).mockImplementation(async (callback: (value: typeof secondTx) => unknown) => callback(secondTx))
  ...
  it('bounds lifecycle reads to generated ids and never scans resolved or dismissed history', async () => {
    ...
    const history = Array.from({ length: 700 }, (_, index) => ({
      id: `history-${index}`, ownerId: 'owner-1', teamId: 'team-1', type: 'HISTORY',
      status: index % 2 ? 'RESOLVED' : 'DISMISSED', feedback: null, ruleVersion: 'old',
      expiresAt: null, resolvedAt: null,
    }))
    tx.intelligenceFinding.findMany.mockImplementation(async (args: { where: { AND: Array<Record<string, unknown>> } }) => {
  ...
  it('reconciles absent OPEN findings in truthful bounded batches', async () => {
    const makeOpen = (count: number, offset = 0) => Array.from({ length: count }, (_, index) => ({
      id: `open-${String(index + offset).padStart(4, '0')}`, ownerId: 'owner-1', teamId: 'team-1',
      type: 'STALE', status: 'OPEN', feedback: null, ruleVersion: 'v1', expiresAt: null, resolvedAt: null,
    }))
    const firstTx = createWriteTx()
    ...
    expect(first.res._getJSONData().coverage).toEqual(expect.objectContaining({
      complete: false, reconciliationComplete: false, reconciliationLimit: FINDING_RECONCILE_LIMIT,
    }))
    expect(first.res._getJSONData().autoResolved).toBe(FINDING_RECONCILE_LIMIT)
    expect(firstTx.intelligenceFinding.findMany).toHaveBeenCalledWith(expect.objectContaining({
      orderBy: { id: 'asc' }, take: FINDING_RECONCILE_LIMIT + 1,
    }))
    expect(firstTx.intelligenceFinding.updateMany.mock.calls[0][0].where.AND[1].id.in)
      .toHaveLength(FINDING_RECONCILE_LIMIT)
    ...
    expect(second.res._getJSONData().coverage).toEqual(expect.objectContaining({
      complete: true, reconciliationComplete: true,
    }))
