...
const finding = {
  ...
  confidence: 0.8,
  score: 300,
  title: 'Vehicle record is stale',
  explanation: 'Review the vehicle status.',
  evidence: [],
  evidenceValid: true,
  evidenceTotal: 0,
  evidenceTruncated: false,
  action: 'Review vehicle',
  actionUrl: '/vehicles?record=v-1',
  feedback: null,
...
describe('Intelligence findings page', () => {
  ...
  it('filters status and follows the signed next cursor without replacing loaded results', async () => {
    global.fetch = jest
      ...
      .mockImplementationOnce(() =>
        json({ findings: [{ ...finding, id: 'f-2', title: 'Second issue' }], pagination: { nextCursor: null } })
      ) as jest.Mock
  ...
  it('keeps canonical client actions and strips every non-canonical variant', async () => {
    const variants = [
      ...
      },
      {
        ...
        id: 'client-query',
        title: 'Client query',
        action: 'Open client query',
        actionUrl: '/clients?record=c-1',
      },
      {
        ...finding,
        id: 'collection-hash',
        title: 'Collection hash',
        action: 'Open hash',
        actionUrl: '/vehicles?record=v-1#private',
      },
      {
        ...finding,
        id: 'nested-client',
        title: 'Nested client',
        action: 'Open nested client',
        actionUrl: '/clients/c-1/notes',
      },
    ]
    global.fetch = jest
      .fn()
    ...
    render(<IntelligencePage />)
    expect(await screen.findByRole('link', { name: 'Open canonical client' })).toHaveAttribute(
      'href',
      '/clients/client%2Fone'
    )
  ...
  it('does not render invalid lifecycle controls in the dismissed tab', async () => {
    global.fetch = jest
      ...
      .mockImplementationOnce(() =>
        json({
          findings: [{ ...finding, status: 'DISMISSED', effectiveStatus: 'DISMISSED' }],
  ...
  it('ignores an old mutation failure after the user changes filters', async () => {
    ...
    global.fetch = jest
      ...
      .mockImplementationOnce(
        () =>
          new Promise((_resolve, reject) => {
            rejectPatch = () => reject(new Error('old failure'))
          })
      )
    ...
    global.fetch = jest
      ...
      .mockImplementationOnce(
        () =>
          new Promise((resolve) => {
            resolveFirst = () =>
              resolve({ ok: true, json: async () => ({ finding: { ...finding, status: 'DISMISSED' } }) })
          })
      )
      .mockImplementationOnce(
        () =>
