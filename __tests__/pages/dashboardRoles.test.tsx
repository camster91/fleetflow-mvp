...
jest.mock('../../lib/session', () => ({
  useSession: () => ({
    data: { user: { name: 'Casey', email: 'c@example.test', role: mockRole } },
...
jest.mock('../../services/apiService', () => ({
  ...
  getDeliveries: jest
    .fn()
    .mockResolvedValue([
      { id: 'd1', customer: 'North Shop', address: '1 Road', status: 'pending', driver: 'Pat', items: 2, progress: 0 },
    ]),
  getMaintenanceTasks: jest
    .fn()
    .mockResolvedValue([
      { id: 'm1', vehicle: 'Unit 7', type: 'Brakes', dueDate: '2026-08-01', priority: 'high', completed: false },
    ]),
}))

describe('role dashboards', () => {
  const dto = (returnedRole: string, deliveryAvailable = true) => ({
    role: returnedRole,
    dashboardRole: ['OWNER', 'ADMIN', 'MANAGER'].includes(returnedRole)
      ? 'admin'
      : returnedRole === 'DISPATCHER'
        ? 'dispatcher'
        : returnedRole === 'TECHNICIAN'
          ? 'maintenance'
          : returnedRole === 'DRIVER'
            ? 'driver'
            : 'viewer',
    ...
    actions:
      returnedRole === 'VIEWER'
        ...
        : returnedRole === 'DRIVER'
          ...
          : [
              { label: 'Act one', href: '/one' },
              { label: 'Act two', href: '/two' },
              { label: 'Act three', href: '/three' },
            ],
    sources: {
      vehicles: {
        available: true,
      ...
      deliveries: {
        ...
        items: deliveryAvailable
          ...
          : [],
        total: deliveryAvailable ? 1 : null,
      },
      maintenance: {
        available: true,
        items: [
  ...
  beforeEach(() => {
    global.fetch = jest.fn().mockImplementation(() => {
      const returnedRole = mockRole
      return Promise.resolve({ ok: true, json: async () => dto(returnedRole) })
    }) as jest.Mock
  })
  ...
  test.each([
    { ok: false, status: 403 },
    { ok: true, json: async () => ({ role: 'ROOT' }) },
  ])('fails closed for denied or malformed canonical context', async (response) => {
  ...
  test('recovers after retrying a partial response', async () => {
    const partial = dto('DISPATCHER', false)
    const healthy = dto('DISPATCHER', true)
    ;(global.fetch as jest.Mock)
      .mockResolvedValueOnce({ ok: true, json: async () => partial })
      .mockResolvedValueOnce({ ok: true, json: async () => healthy })
    render(<Dashboard />)
    fireEvent.click(await screen.findByRole('button', { name: 'Retry loading dashboard' }))
    await waitFor(() =>
      expect(screen.queryByRole('button', { name: 'Retry loading dashboard' })).not.toBeInTheDocument()
    )
  })
  ...
  test('ignores an older dashboard request that completes after a newer retry', async () => {
    ...
    const old = new Promise((resolve) => {
      ...
      newer = new Promise((resolve) => {
        resolveNew = resolve
      })
    ;(global.fetch as jest.Mock)
      .mockResolvedValueOnce({ ok: true, json: async () => dto('DISPATCHER', false) })
    ...
    fireEvent.click(retry)
    resolveNew({ ok: true, json: async () => ({ ...dto('DISPATCHER'), decisions: ['Newest', 'Two', 'Three'] }) })
    await screen.findByText('Newest')
    resolveOld({ ok: true, json: async () => ({ ...dto('DISPATCHER'), decisions: ['Stale', 'Two', 'Three'] }) })
    await Promise.resolve()
    expect(screen.queryByText('Stale')).not.toBeInTheDocument()
  })

  test.each(['OWNER', 'DISPATCHER', 'TECHNICIAN', 'DRIVER', 'VIEWER'])(
    'renders a truthful empty state for %s',
    async (nextRole) => {
      const empty = dto(nextRole)
      for (const source of Object.values(empty.sources)) {
        source.items = []
        source.total = 0
      }
      ;(global.fetch as jest.Mock).mockResolvedValue({ ok: true, json: async () => empty })
      render(<Dashboard />)
