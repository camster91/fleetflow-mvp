...
jest.mock('@/lib/session', () => ({
  useSession: () => ({ data: { user: { id: 'u1', name: 'Casey', role: 'user' } }, status: 'authenticated' }),
}))
jest.mock('@/services/apiService', () => ({
  ...
  getSOPCategories: jest.fn(),
  getVendingMachines: jest.fn(),
  deleteVehicle: jest.fn(),
  deleteDelivery: jest.fn(),
  updateDelivery: jest.fn(),
  updateMaintenanceTask: jest.fn(),
  deleteSOPCategory: jest.fn(),
...
beforeEach(() => {
  ...
  ;(api.getVendingMachines as jest.Mock).mockResolvedValue([
    { id: 'vm1', name: 'Lobby', location: 'HQ', status: 'active' },
  ])
...
describe('list pages hide create/edit/delete from roles that cannot manage', () => {
  const pages = [
    {
      ...
      readers: ['DISPATCHER', 'DRIVER', 'VIEWER'],
    },
    {
      name: 'deliveries',
      Page: DeliveriesPage,
      record: 'North Shop',
      add: /New Delivery/,
      managers: ['OWNER', 'MANAGER', 'DISPATCHER'],
      readers: ['DRIVER', 'VIEWER'],
    },
    {
      name: 'clients',
      Page: ClientsPage,
    ...
    },
    {
      ...
      record: null,
      add: /Add Task/,
      managers: ['OWNER', 'MANAGER', 'TECHNICIAN'],
      readers: ['DRIVER', 'VIEWER'],
    },
    {
      name: 'sop',
      ...
      add: /New Category/,
      managers: ['OWNER', 'MANAGER'],
      readers: ['TECHNICIAN', 'DRIVER', 'VIEWER'],
    },
    {
      name: 'vending machines',
      Page: VendingMachinesPage,
      record: 'Lobby',
      add: /Add Machine/,
      managers: ['OWNER', 'MANAGER'],
      readers: ['VIEWER', 'MEMBER'],
    },
  ]
  ...
  for (const { name, Page, record, add, managers, readers } of pages) {
    ...
    it.each([...readers, null])(`${name}: %s gets no create, edit or delete action`, async (role) => {
      ...
      expect(
        screen.queryByRole('button', { name: /Mark Delivered|Set Maintenance|Set Active/ })
...
describe('/team controls follow the caller role', () => {
  const members = [
    {
      ...
      joinedAt: '2026-01-01',
      user: { id: 'u-o', name: 'Olive Owner', email: 'owner@example.test', image: null },
      invitedByUser: null,
    },
    {
      id: 'm-admin',
      ...
      invitedAt: '2026-01-01',
      joinedAt: '2026-01-01',
      user: { id: 'u-a', name: 'Ada Admin', email: 'admin@example.test', image: null },
      invitedByUser: null,
    },
    {
      id: 'm-driver',
      ...
      invitedAt: '2026-01-01',
      joinedAt: '2026-01-01',
      user: { id: 'u-d', name: 'Dee Driver', email: 'driver@example.test', image: null },
      invitedByUser: null,
    },
  ]
  beforeEach(() => {
  ...
  it.each(['DISPATCHER', 'TECHNICIAN', 'DRIVER'])(
    '%s gets a notice and the member list is never requested',
    async (role) => {
      mockRole = role
      renderPage(TeamPage)
...
describe('/billing for roles that cannot view billing', () => {
  it.each(['DISPATCHER', 'TECHNICIAN', 'DRIVER', 'MEMBER', 'VIEWER'])(
    '%s sees the read-only free-beta notice without calling billing endpoints',
    async (role) => {
      mockRole = role
      render(<BillingPage />)
  ...
  it('treats a 403 from the status endpoint as not a billing viewer, not an error', async () => {
    ...
    global.fetch = jest.fn((url: string) =>
      url === '/api/subscription/status'
        ...
        : jsonResponse({ available: false, pricing: null })
...
describe('/documents after a document is CONFIRMED', () => {
  const extraction = {
    ...
    fields: { vendor: { value: 'Honest Garage', confidence: 0.9, citationIds: [] } },
    services: [],
    parts: [],
    citations: [],
    warnings: [],
...
describe('a rejected delete keeps the confirm dialog open', () => {
  const cases = [
    ...
    },
    {
      ...
      Page: SopPage,
      record: 'Safety',
      trigger: 'Delete Safety',
      title: 'Delete Category',
      remove: () => api.deleteSOPCategory,
    },
    {
      name: 'vending machines',
      Page: VendingMachinesPage,
      record: 'Lobby',
      trigger: 'Delete Lobby',
      title: 'Delete Machine',
      remove: () => api.deleteVendingMachine,
    },
  ]
  it.each(cases)('$name', async ({ Page, record, trigger, title, remove }) => {
    ...
    await waitFor(() =>
      expect(
        within(screen.getByRole('alertdialog', { name: title })).getByRole('button', { name: 'Cancel' })
      ).toBeEnabled()
    )
