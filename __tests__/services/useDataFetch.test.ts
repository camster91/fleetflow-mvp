...
describe('useDataFetch', () => {
  it('does not let an out-of-order earlier response overwrite newer data', async () => {
    ...
    await act(async () => { second.resolve('newer'); await refetching })
  ...
  it('reloads when deps change and ignores the superseded request', async () => {
    ...
    const calls: string[] = []
    const { result, rerender } = renderHook(({ id }) => useDataFetch(
      () => { calls.push(id); return id === 'a' ? slow.promise : Promise.resolve(`value-${id}`) },
      'initial',
      [id],
    ), { initialProps: { id: 'a' } })
    ...
    await act(async () => { slow.resolve('value-a'); await slow.promise })
