...
describe('controlled pilot contracts', () => {
  it('accepts only content-free, bounded event signals', () => {
    ...
    expect(
      pilotEventSchema.safeParse({ eventType: 'DASHBOARD_OPENED', sessionKey: 'short', prompt: 'customer content' })
        .success
    ).toBe(false)
  })
  ...
  it('requires consent before activation and validates the pilot date range', () => {
    ...
    expect(
      enrollmentSchema.safeParse({
        status: 'ACTIVE',
        consent: true,
        pilotStartsAt: '2026-08-10T00:00:00.000Z',
        pilotEndsAt: '2026-08-09T00:00:00.000Z',
      }).success
    ).toBe(false)
    expect(
      enrollmentSchema.safeParse({ status: 'ACTIVE', consent: true, supportOwnerLabel: 'Pilot support' }).success
    ).toBe(true)
  })
