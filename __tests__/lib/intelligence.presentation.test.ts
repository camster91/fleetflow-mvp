...
const base = {
  ...
  ruleVersion: 'v1',
  title: 'Review',
  explanation: 'Review record',
  action: 'Review',
  status: 'OPEN',
  feedback: null,
  generatedAt: new Date('2026-08-08T16:00:00Z'),
  expiresAt: null,
  resolvedAt: null,
}
...
describe('intelligence presentation boundary', () => {
  it.each([
    ...
    ['/clients?record=c-1', null],
    ['/clients/c-1/notes', null],
    ['/vehicles/v-1', null],
    ['//evil.example/vehicles?record=x', null],
    ['/settings?record=x', null],
  ...
  it('redacts schema-valid legacy PII and unknown strings without exposing raw JSON', () => {
    const presented = presentStoredFinding({
      ...base,
