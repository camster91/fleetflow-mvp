...
} = require('@/scripts/backup-restore-lib.cjs')
...
describe('backup and isolated restore verifier safety contract', () => {
  test('requires explicit safe source and absolute backup directory', () => {
    ...
    expect(() => parseArgs(['--source-container', 'fleetflow-postgres', '--backup-dir', 'relative'])).toThrow(
      /absolute/
    )
    expect(() => parseArgs(['--source-container', 'fleetflow;rm', '--backup-dir', '/secure/backups'])).toThrow(
      /unsupported/
    )
    expect(parseArgs(['--source-container', 'fleetflow-postgres', '--backup-dir', '/secure/backups'])).toEqual({
      sourceContainer: 'fleetflow-postgres',
      backupDir: expect.stringMatching(/secure[\\/]backups$/),
    })
  })
  ...
  test('requires migration, core-record, and ownership integrity evidence', () => {
    const valid = {
      publicTableCount: 47,
      criticalRowCounts: Object.fromEntries(
        REQUIRED_RESTORE_TABLES.map((table: string) => [table, table === 'User' ? 1 : 0])
      ),
