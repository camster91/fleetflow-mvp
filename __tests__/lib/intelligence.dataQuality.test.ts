...
describe('assessDataQuality', () => {
  it('reports status-aware missing fleet data with stable safe links', () => {
    ...
    records.vehicles.push({
      id: 'vehicle-1', status: 'active', mileage: 0, driver: null,
      lastService: null, nextService: null, updatedAt: recent, lastUpdated: recent,
      createdAt: new Date('2026-07-01T00:00:00.000Z'),
    })
    records.deliveries.push({
      id: 'delivery-1', status: 'in-transit', vehicleId: null, driver: ' ',
      scheduledTime: null, estimatedArrival: null, contactPerson: null, updatedAt: recent,
    })
    ...
    expect(result).toEqual(expect.arrayContaining([
      {
        id: 'vehicle:vehicle-1:mileage', entityType: 'vehicle', entityId: 'vehicle-1',
        severity: 'high', field: 'mileage', message: 'Active vehicle needs a positive mileage reading.',
    ...
    expect(result.every((issue) => /^\/(vehicles|deliveries|maintenance|clients)(\?|\/)/.test(issue.actionUrl))).toBe(true)
  })

  it('does not apply active-work rules to terminal or cancelled records', () => {
    const records = emptyRecords()
    records.vehicles.push({
      id: 'inactive', status: 'inactive', mileage: 0, driver: null,
    ...
    for (const status of ['delivered', 'failed', 'cancelled']) {
      records.deliveries.push({
        id: status, status, vehicleId: null, driver: null, scheduledTime: null,
        estimatedArrival: null, contactPerson: null,
  ...
  it('treats picked-up deliveries as urgent active work', () => {
    const records = emptyRecords()
    records.deliveries.push({
      id: 'picked-up', status: 'picked-up', vehicleId: null, driver: null,
    ...
    expect(assessDataQuality(records, NOW)).toEqual(expect.arrayContaining([
  ...
  it('treats zero actual cost as recorded but flags a missing completed cost', () => {
    ...
    records.maintenance.push(
      { id: 'zero', completed: true, dueDate: recent, vehicleId: 'v1', costEstimate: null, actualCost: 0, updatedAt: recent },
      { id: 'missing', completed: true, dueDate: recent, vehicleId: 'v1', costEstimate: 100, actualCost: null, updatedAt: recent },
    )

    const result = assessDataQuality(records, NOW)
    expect(result).toEqual([
      expect.objectContaining({ id: 'maintenance:missing:actualCost', severity: 'medium' }),
    ])
  })

  it('gives a new active vehicle a seven-day grace period for default zero mileage', () => {
    const base = {
      status: 'active', mileage: 0, driver: 'Driver', lastService: recent, nextService: recent,
      updatedAt: recent, lastUpdated: recent,
    }
    const records = emptyRecords()
    records.vehicles.push(
    ...
    const mileageIds = assessDataQuality(records, NOW)
      .filter((item) => item.field === 'mileage').map((item) => item.entityId)
  ...
  it('uses documented deterministic stale thresholds for active records', () => {
    const records = emptyRecords()
    records.vehicles.push({
      id: 'stale-vehicle', status: 'active', mileage: 10, driver: 'Driver',
      createdAt: new Date('2020-01-01'),
      lastService: recent, nextService: recent,
      updatedAt: recent,
      lastUpdated: new Date(NOW.getTime() - DATA_QUALITY_THRESHOLDS.activeVehicleStaleMs - 1),
    })
    records.deliveries.push({
      id: 'stale-delivery', status: 'in-transit', vehicleId: 'v1', driver: 'Driver',
      scheduledTime: recent, estimatedArrival: recent, contactPerson: '{}',
      updatedAt: new Date(NOW.getTime() - DATA_QUALITY_THRESHOLDS.inTransitDeliveryStaleMs - 1),
    })
    records.maintenance.push({
      id: 'stale-maintenance', completed: false,
      dueDate: new Date(NOW.getTime() - 86400000), vehicleId: 'v1',
      costEstimate: null, actualCost: null,
