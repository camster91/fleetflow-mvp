...
const findings: SafeFinding[] = [
  {
    ...
    explanation: 'Three days overdue',
  ...
  },
  {
    id: 'f-low',
    severity: 'low',
    title: 'Mileage missing',
    explanation: 'Mileage is absent',
    recommendedAction: 'Add mileage',
    score: 20,
  },
]
...
describe('provider-neutral fleet summary service', () => {
  it('exports a strict request payload schema and keeps AbortSignal outside it', async () => {
    ...
    expect(fleetSummaryRequestPayloadSchema.safeParse({ findings, signal: new AbortController().signal }).success).toBe(
      false
    )
    expect(fleetSummaryRequestPayloadSchema.safeParse({ findings, tenantId: 'must-not-enter-domain' }).success).toBe(
      false
    )

    const result = await generateFleetSummary({ findings, tenantId: 'x' } as never, {
      env: openAiEnv,
      dependencies: deps(jest.fn()),
    })
  ...
  it('refuses to run the provider layer in a browser runtime', async () => {
    ...
    try {
      await expect(
        generateFleetSummary({ findings }, { env: { AI_PROVIDER: 'disabled' }, dependencies: deps(jest.fn()) })
  ...
  it('uses deterministic grounded fallback when AI is disabled', async () => {
    const result = await generateFleetSummary(
      { findings },
      { env: { AI_PROVIDER: 'disabled' }, dependencies: deps(jest.fn()) }
    )
  ...
  it('selects a registered third-party provider without core edits or OpenAI configuration', async () => {
    ...
    const result = await generateFleetSummary(
      { findings },
      {
        env: { AI_PROVIDER: 'local-test', CUSTOM_ENDPOINT: 'internal' },
        dependencies: deps(jest.fn()),
        registry,
      }
    )
  ...
  it.each(['', 'UPPER', 'has space', 'x'.repeat(33), '__proto__', 'constructor', 'prototype'])(
    'rejects invalid or reserved provider name %p',
    (name) => {
      expect(() =>
        new AiProviderRegistry().register(name, () => ({
          name,
          model: null,
          generate: async () => ({
            content: validOutput(),
  ...
  it('rejects duplicate provider registration', () => {
    const factory = () => ({
      name: 'custom',
      model: null,
      generate: async () => ({ content: validOutput(), usage: { inputTokens: 0, outputTokens: 0, totalTokens: 0 } }),
    })
  ...
  it('posts a non-retained strict Responses API request and normalizes usage', async () => {
    const fetchMock = jest.fn(async (_url, init) => {
      ...
      expect(body).toMatchObject({
        ...
        text: { format: { type: 'json_schema', name: 'fleet_summary', strict: true } },
      })
  ...
  it('never sends embedded sensitive patterns to the provider', async () => {
    ...
    const result = await generateFleetSummary(
      ...
      { env: openAiEnv, dependencies: deps(fetchMock) }
    )
  ...
  it.each([
    [
      'unknown citation',
      { ...validOutput(), claims: [{ text: 'Invented', citationIds: ['nope'] }] },
      'citation_invalid',
    ],
    ['missing citation', { ...validOutput(), claims: [{ text: 'Uncited', citationIds: [] }] }, 'output_invalid'],
    [
      'duplicate citation',
      { ...validOutput(), claims: [{ text: 'Repeated', citationIds: ['f-high', 'f-high'] }] },
      'output_invalid',
    ],
  ...
  it.each([
    ...
    [
      'excessive output items',
      {
        ...validOutput(),
        actions: new Array(9).fill(0).map((_, i) => ({ text: `Action ${i}`, citationIds: ['f-high'] })),
      },
    ],
  ...
  it.each([
    [
      'malformed JSON',
      { id: 'x', status: 'completed', output: [{ type: 'message', content: [{ type: 'output_text', text: '{' }] }] },
      'output_malformed',
    ],
    [
      'refusal',
  ...
  it('retries only transient statuses with bounded delay', async () => {
    ...
    const result = await generateFleetSummary(
      { findings },
      { env: openAiEnv, dependencies: deps(fetchMock, { sleep }) }
    )
    expect(result.meta.status).toBe('generated')
  ...
  it.each([
    ...
    const fetchMock = jest.fn(async () =>
      response({ error: { message: 'do not log me' } }, status)
    ) as unknown as typeof fetch
  ...
  it('times out and aborts the provider request', async () => {
    ...
    const result = await generateFleetSummary(
      { findings },
      { env: { ...openAiEnv, AI_TIMEOUT_MS: '10' }, dependencies: deps(fetchMock) }
    )
  ...
  it('honours caller cancellation without retrying', async () => {
    ...
    const promise = generateFleetSummary(
      { findings },
      { env: openAiEnv, signal: controller.signal, dependencies: deps(fetchMock) }
    )
    controller.abort()
  ...
  it('classifies cancellation while waiting to retry', async () => {
    ...
    const result = await generateFleetSummary(
      { findings },
      { env: openAiEnv, signal: controller.signal, dependencies: deps(fetchMock, { sleep }) }
    )
    expect(result.meta.fallbackReason).toBe('caller_aborted')
  ...
  it('times out a hanging custom provider at the orchestration boundary', async () => {
    ...
    const result = await generateFleetSummary(
      { findings },
      {
        env: { AI_PROVIDER: 'hanging', AI_TIMEOUT_MS: '10' },
        dependencies: deps(jest.fn()),
        registry,
      }
    )
  ...
  it('cancels a hanging custom provider from the transport signal', async () => {
    ...
    const registry = new AiProviderRegistry().register('hanging', () => provider)
    const pending = generateFleetSummary(
      { findings },
      {
        env: { AI_PROVIDER: 'hanging' },
  ...
  it('cleans up the orchestration timeout after provider completion', async () => {
    ...
    const result = await generateFleetSummary(
      { findings },
      {
        env: { AI_PROVIDER: 'instant' },
        dependencies: deps(jest.fn(), { setTimer, clearTimer }),
        registry,
      }
    )
    expect(result.meta.status).toBe('generated')
  ...
  it('rejects oversized input before a provider call', async () => {
    ...
    const result = await generateFleetSummary(
      { findings },
      { env: { ...openAiEnv, AI_MAX_INPUT_CHARS: '20' }, dependencies: deps(fetchMock) }
    )
  ...
  it('classifies malformed runtime input without calling a provider', async () => {
    const fetchMock = jest.fn() as unknown as typeof fetch
    const result = await generateFleetSummary(
      { findings: [{ ...findings[0], unexpected: 'not allowed' }] as never },
      { env: openAiEnv, dependencies: deps(fetchMock) }
    )
    expect(result.meta.fallbackReason).toBe('input_invalid')
    expect(fetchMock).not.toHaveBeenCalled()
  ...
  it('cancels a declared oversized response before returning fallback', async () => {
    ...
    const result = await generateFleetSummary(
      { findings },
      {
        env: { ...openAiEnv, AI_MAX_RESPONSE_BYTES: '100' },
        dependencies: deps(fetchMock),
      }
    )
    expect(result.meta.fallbackReason).toBe('response_too_large')
  ...
  it('does not perform an unbounded drain when error-body cancellation fails', async () => {
    ...
    const result = await generateFleetSummary(
      ...
      {
        env: openAiEnv,
        dependencies: deps(jest.fn(async () => providerResponse) as unknown as typeof fetch),
      }
    )
  ...
  it('uses injected time so fallback and metadata are deterministic', async () => {
    const result = await generateFleetSummary(
      { findings },
      { env: { AI_PROVIDER: 'disabled' }, dependencies: deps(jest.fn(), { now: () => 4_321 }) }
    )
