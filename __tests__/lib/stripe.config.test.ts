import { getBillingAvailability, getCanonicalAppUrl, getConfiguredPricing, retrieveInvoiceSnapshot, retrieveSubscriptionSnapshot, stripe } from '@/lib/stripe'

const NAMES = [
  'NEXTAUTH_URL', 'APP_URL', 'STRIPE_SECRET_KEY', 'STRIPE_WEBHOOK_SECRET', 'STRIPE_PRICE_MONTHLY',
  'STRIPE_PRICE_YEARLY', 'STRIPE_PRICE_MONTHLY_AMOUNT', 'STRIPE_PRICE_YEARLY_AMOUNT', 'STRIPE_PRICE_CURRENCY',
] as const
...
describe('Stripe launch configuration', () => {
  ...
  it.each(['http://fleetvera.example', 'javascript:alert(1)', 'https://user:pass@fleetvera.example', 'https://fleetvera.example?redirect=evil'])('rejects unsafe canonical app URL %s', value => {
  ...
  it('requires exact integer minor-unit pricing and an allowed currency', () => {
    ...
    expect(getConfiguredPricing()).toEqual({ monthly: { amount: 4900, currency: 'CAD' }, yearly: { amount: 49000, currency: 'CAD' } })
  ...
  it('is unavailable unless Stripe, pricing, and HTTPS origin are all valid', () => {
    Object.assign(process.env, {
      NEXTAUTH_URL: 'https://fleetvera.example', STRIPE_SECRET_KEY: 'sk_test', STRIPE_WEBHOOK_SECRET: 'whsec_test',
      STRIPE_PRICE_MONTHLY: 'price_monthly', STRIPE_PRICE_YEARLY: 'price_yearly',
      STRIPE_PRICE_MONTHLY_AMOUNT: '4900', STRIPE_PRICE_YEARLY_AMOUNT: '49000', STRIPE_PRICE_CURRENCY: 'CAD',
    })
  ...
  it('returns a provider-neutral subscription snapshot', async () => {
    ...
    jest.spyOn(stripe.subscriptions, 'retrieve').mockResolvedValueOnce({
      id: 'sub_123', status: 'past_due', cancel_at_period_end: true,
    ...
    await expect(retrieveSubscriptionSnapshot('sub_123')).resolves.toEqual({
      ...
      currentPeriodEnd: 200, cancelAtPeriodEnd: true,
    })
  })

  it('represents a provider resource_missing response as cancelled', async () => {
    process.env.STRIPE_SECRET_KEY = 'sk_test'
    jest.spyOn(stripe.subscriptions, 'retrieve').mockRejectedValueOnce(Object.assign(new Error('raw provider message'), { code: 'resource_missing' }))
    await expect(retrieveSubscriptionSnapshot('sub_missing')).resolves.toEqual({
      id: 'sub_missing', status: 'CANCELLED', priceId: null, currentPeriodStart: null,
  ...
  it.each([
    ...
    jest.spyOn(stripe.invoices, 'retrieve').mockResolvedValueOnce({
      id: 'in_123', ...state, amount_paid: 0, amount_due: 4900, currency: 'cad', invoice_pdf: null,
      period_start: 100, period_end: 200,
    } as never)
    await expect(retrieveInvoiceSnapshot('in_123')).resolves.toEqual(expect.objectContaining({ status: expected, amountPaid: 0, amountDue: 4900 }))
  })
