...
describe('deterministic intelligence rules', () => {
  describe('maintenance schedule', () => {
    it('distinguishes overdue and due-soon tasks and links directly to each record', () => {
      const findings = maintenanceScheduleFindings(input({ maintenance: [
      ...
      expect(findings.find(finding => finding.evidence[0].entityId === 'overdue/id')?.actionUrl).toBe('/maintenance?record=overdue%2Fid')
  ...
  describe('maintenance cost concentration', () => {
    const completed = (id: string, vehicleId: string, actualCost: number, completedDate = NOW) => ({
      id, vehicleId, actualCost, completedDate, completed: true,
    })

    it('requires repeated, recent actual costs and uses integer cents', () => {
      const findings = maintenanceCostFindings(input({ maintenance: [
      ...
      expect(findings[0]).toMatchObject({ type: 'maintenance-cost-concentration', actionUrl: '/vehicles?record=vehicle-a' })
    ...
    it('rounds fractional cents explicitly before doing money comparisons', () => {
      const findings = maintenanceCostFindings(input({ maintenance: [
        completed('a', 'v', 499.995),
        completed('b', 'v', 499.995),
        completed('c', 'v', 0.005),
      ] }))
  ...
  describe('vehicle freshness', () => {
    it('applies status-specific thresholds and ignores non-operational vehicles', () => {
      const findings = staleVehicleFindings(input({ vehicles: [
      ...
      expect(findings.map(finding => finding.evidence[0].entityId)).toEqual([
        'active-stale', 'delayed-stale', 'maintenance-stale',
      ])
  ...
  describe('delivery operational rules', () => {
    it('flags active deliveries only after the scheduled-time grace boundary', () => {
      ...
      const findings = deliveryScheduleFindings(input({ deliveries: [
    ...
    it('counts active deliveries per vehicle at the documented load threshold', () => {
      const deliveries = Array.from({ length: 10 }, (_, index) => ({
        id: `delivery-${index}`, status: index === 9 ? 'delivered' : 'pending', vehicleId: index < 5 ? 'v/1' : 'v2',
      }))
    ...
    it('includes delayed deliveries in vehicle load and assignment rules', () => {
      const delayed = Array.from({ length: 5 }, (_, index) => ({
        id: `delayed-${index}`, status: 'delayed', driver: index === 0 ? null : 'Driver', vehicleId: 'v1',
      }))
    ...
    it('creates one finding when an active delivery lacks driver, vehicle, or both', () => {
      const findings = unassignedDeliveryFindings(input({ deliveries: [
  ...
  describe('data-quality blockers', () => {
    it('deduplicates by source field, keeps the strongest severity, and ignores caller URLs/messages', () => {
      const findings = dataQualityBlockerFindings(input({ dataQualityIssues: [
        { entityType: 'vehicle', entityId: 'vehicle/1', field: 'mileage', severity: 'low', actionUrl: 'https://evil.invalid' },
      ...
      expect(findings.find(finding => finding.evidence[0].field === 'mileage')).toMatchObject({
        severity: 'high', actionUrl: '/vehicles?record=vehicle%2F1',
      })
      expect(findings.find(finding => finding.evidence[0].field === 'record')?.explanation).not.toContain('secret-token')
  ...
  it('rejects an invalid clock rather than silently using wall-clock time', () => {
    expect(() => maintenanceScheduleFindings({ ...input({}), now: new Date('invalid') })).toThrow('now must be a valid Date')
  })
  ...
  it('rejects ambiguous or invalid string dates in every timestamp-driven rule', () => {
    ...
    for (const value of ambiguous) {
      expect(maintenanceScheduleFindings(input({ maintenance: [
        { id: 'm', completed: false, dueDate: value },
      ] }))).toEqual([])
      expect(maintenanceCostFindings(input({ maintenance: [
      ...
      expect(staleVehicleFindings(input({ vehicles: [
        { id: 'v', status: 'active', lastUpdated: value },
      ] }))).toEqual([])
      expect(deliveryScheduleFindings(input({ deliveries: [
        { id: 'd', status: 'pending', scheduledTime: value },
      ] }))).toEqual([])
    }
  })
  ...
  it('does not place ambiguous update timestamps into assignment or load evidence', () => {
    const deliveries = Array.from({ length: 5 }, (_, index) => ({
      id: `d-${index}`, status: 'delayed', driver: index ? 'Driver' : null,
      vehicleId: 'v', updatedAt: '2026-08-08T12:00:00',
    }))
  ...
  it('normalizes equivalent explicit-offset instants across timestamp-driven rules', () => {
    ...
    const scenarios = [
      (value: string) => maintenanceScheduleFindings(input({ maintenance: [{ id: 'm', completed: false, dueDate: value }] })),
      (value: string) => maintenanceCostFindings(input({ maintenance: [
        { id: 'm1', vehicleId: 'v', completed: true, completedDate: value, actualCost: 400 },
        { id: 'm2', vehicleId: 'v', completed: true, completedDate: value, actualCost: 400 },
        { id: 'm3', vehicleId: 'v', completed: true, completedDate: value, actualCost: 400 },
      ] })),
      (value: string) => staleVehicleFindings(input({ vehicles: [{ id: 'v', status: 'active', lastUpdated: value }] })),
      (value: string) => deliveryScheduleFindings(input({ deliveries: [{ id: 'd', status: 'pending', scheduledTime: value }] })),
    ]
  ...
  it('drops oversized, whitespace-padded, and control-character source IDs', () => {
    ...
    for (const id of invalidIds) {
      expect(maintenanceScheduleFindings(input({ maintenance: [{ id, completed: false, dueDate: new Date(NOW.getTime() - DAY) }] }))).toEqual([])
      expect(staleVehicleFindings(input({ vehicles: [{ id, status: 'active', lastUpdated: new Date(NOW.getTime() - 8 * DAY) }] }))).toEqual([])
      expect(deliveryScheduleFindings(input({ deliveries: [{ id, status: 'pending', scheduledTime: new Date(NOW.getTime() - DAY) }] }))).toEqual([])
      expect(dataQualityBlockerFindings(input({ dataQualityIssues: [{ entityType: 'vehicle', entityId: id, field: 'mileage', severity: 'high' }] }))).toEqual([])
    }
  })

  it.each([
    {
      name: 'maintenance schedule', rule: maintenanceScheduleFindings, key: 'maintenance',
      records: [
    ...
    {
      name: 'maintenance costs', rule: maintenanceCostFindings, key: 'maintenance',
      records: [
    ...
    {
      name: 'vehicle freshness', rule: staleVehicleFindings, key: 'vehicles',
      records: [
    ...
    {
      name: 'delivery schedule', rule: deliveryScheduleFindings, key: 'deliveries',
      records: [
    ...
    {
      name: 'delivery load', rule: deliveryLoadFindings, key: 'deliveries',
      records: [...Array.from({ length: 5 }, (_, i) => ({ id: `z${i}`, status: 'pending', vehicleId: 'z' })),
        ...Array.from({ length: 5 }, (_, i) => ({ id: `a${i}`, status: 'pending', vehicleId: 'a' }))],
    },
    {
      name: 'unassigned delivery', rule: unassignedDeliveryFindings, key: 'deliveries',
      records: [
    ...
    {
      name: 'data quality', rule: dataQualityBlockerFindings, key: 'dataQualityIssues',
      records: [
    ...
    const shuffledRecords = [...records].sort((a, b) => JSON.stringify(a).length - JSON.stringify(b).length || JSON.stringify(a).localeCompare(JSON.stringify(b)))
