...
describe('Ask Fleetvera query planner', () => {
  ...
  it.each(['Summarize activity from 2026-02-30 to 2026-03-01', 'Summarize activity from 2026-13-01 to 2026-13-02'])(
    'rejects normalized invalid ISO dates: %s',
    (question) => {
  ...
  it('uses a separately validated selected entity instead of extracting arbitrary prompt text', () => {
    expect(planFleetQuestion('Summarize the selected vehicle', { type: 'vehicle', id: 'van-12' })).toEqual({
      supported: true,
      intent: 'vehicle_summary',
      entityId: 'van-12',
    })
    expect(planFleetQuestion('Summarize the selected client', { type: 'client', id: 'client-9' })).toEqual({
      supported: true,
