...
function fakeDb(row: Row) {
  const matches = (where: any) => {
    ...
    if ('lockedUntil' in where) {
      ...
      if (f === null) { if (row.lockedUntil !== null) return false }
      else if (f.lte) { if (!row.lockedUntil || row.lockedUntil > f.lte) return false }
    }
    if (where.failedLoginAttempts?.gte !== undefined && row.failedLoginAttempts < where.failedLoginAttempts.gte) return false
    return true
  }
...
describe('recordFailedAttempt', () => {
  ...
  it('counts every concurrent failure and locks exactly once, revoking sessions', async () => {
    ...
    const results = await Promise.all(Array.from({ length: LOGIN_MAX_FAILED_ATTEMPTS + 2 }, () => recordFailedAttempt(db as never, 'u1', now)))
  ...
  it('restarts the count after an expired lock so one failure does not re-lock', async () => {
    const db = fakeDb({ id: 'u1', failedLoginAttempts: 5, lockedUntil: new Date(now.getTime() - 1000), tokenVersion: 1 })
    const result = await recordFailedAttempt(db as never, 'u1', now)
    expect(result.locked).toBe(false)
