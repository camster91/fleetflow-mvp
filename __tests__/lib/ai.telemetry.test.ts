...
describe('AI telemetry privacy and bounds', () => {
  test('accepts bounded metadata and aggregates without raw content or identifiers', () => {
    const first = parseAiTelemetryEvent({
      provider: 'openai',
      modelVersion: 'gpt-safe@2026-08',
      status: 'generated',
      latencyMs: 120,
      inputTokens: 10,
      outputTokens: 4,
    ...
    const second = parseAiTelemetryEvent({
      provider: 'openai',
      modelVersion: 'gpt-safe@2026-08',
      status: 'fallback',
      errorCode: 'timeout',
    ...
    expect(aggregateAiTelemetry([first, second])).toEqual(
