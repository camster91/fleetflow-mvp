...
const InvoicesPage: React.FC = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      case 'uncollectible':
      case 'void': return <XCircle className="h-5 w-5 text-red-500" />;
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-slate-500 mt-1">Your invoices will appear here once you have a paid subscription.</p>
            </div>
          ) : (
            <>
              {/* Mobile card list */}
                  ...
                  <div key={invoice.id} className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-900">#{invoice.id.slice(-8).toUpperCase()}</p>
                    ...
                    <div className="mt-2 flex items-end justify-between">
                      <div>
                        <p className="text-base font-semibold text-slate-900">{formatAmount(invoice.amount, invoice.currency)}</p>
                      ...
                      {invoice.invoicePdf ? (
                        <a href={invoice.invoicePdf} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium min-h-[40px]"
                      ...
                      <tr key={invoice.id} className="hover:bg-slate-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            ...
                            <div>
                              <p className="text-sm font-medium text-slate-900">Invoice #{invoice.id.slice(-8).toUpperCase()}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-semibold text-slate-900">
                  {formatAmount(
                    invoices.filter(inv => inv.status.toLowerCase() === 'paid').reduce((sum, inv) => sum + inv.amount, 0),
                    'USD'
                  )}
                </p>
              </div>
              <div>
                ...
                <p className="text-2xl font-semibold text-slate-900">{new Date(invoices[0]?.createdAt).toLocaleDateString()}</p>
              </div>
            </div>
          </div>
        )}
