...
For each backup, record the database identifier, UTC timestamp, tool/provider, encryption status, retention date, and checksum or provider job ID. The verifier proves that the encrypted dump restores, required tables exist, at least one user and completed migration are present, migrations are not unfinished, and team ownership/references are internally consistent. It intentionally does not prove durable off-host retention, recovery timing, application behavior, or tenant-level business-record completeness. Those still require a separately retained artifact, `npx prisma migrate status`, representative source-versus-restore counts, tenant isolation queries, and an authenticated application smoke test.
