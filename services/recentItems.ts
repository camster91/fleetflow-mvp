...
const addToCategory = (items: RecentItems, category: keyof RecentItems, value: string): RecentItems => {
...
export const addRecentClient = (client: { businessName?: string; name?: string; type?: string }) => {
  const items = getRecentItems()
  let updated = items
...
export const addRecentMaintenance = (task: { vehicle?: string; type?: string }) => {
  const items = getRecentItems()
  let updated = items
...
export const addRecentVendingMachine = (machine: { location?: string; type?: string }) => {
  const items = getRecentItems()
  let updated = items
...
export const searchRecentItems = (category: keyof RecentItems, query: string): string[] => {
  const items = getRecentItems()
  ...
  return categoryItems.filter((item) => item.toLowerCase().includes(lowerQuery))
}
